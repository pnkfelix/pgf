...
\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/basiclayer/Attic/pgfbasetransparency.code.tex,v 1.1 2007/12/12 16:57:17 tantau Exp $
...
\def\pgfdeclarefading#1#2{%
  \setbox\pgfutil@tempboxa=\hbox{{\pgfutil@color{white}\ignorespaces#2}}%
...
% mask will be rotated by #2 (around its center) and then rescaled so
% that it completely covers the path. Then the mask is made part of
% the graphic state (as with \pgfsetfading). The path used for
% computation purposes only -- it is not drawn or otherwise used.
%
% If an additional transform code has been set using
% \pgfsetadditionalfadingtransform, this additional code will
% also be executed.
%
...
% {\tikz[left color=transparent!0,right color=transparent!00] (0,0) rectangle (100bp,100bp);}
%
% \pgfpathmoveto{\pgforigin}
% \pgfpathlineto{\pgfxy(1,0)}
% \pgfpathlineto{\pgfxy(1,1)}
% \pgfsetfadingforcurrentpath{fade right}{0}
% \pgfusepath{fill}

\def\pgfsetfadingforcurrentpath#1#2{%
  \ifdim\pgf@pathminx=16000pt%
    \PackageWarning{pgf}{No path specified for mask size coputation}%
  \else%
    \pgfsetfading{#1}{%
      % Calculate center:
      ...
      \pgf@xa=-\pgf@x%
      \pgftransformcm{\pgfshade@cos}{\pgfshade@sin}{\pgf@sys@tonumber{\pgf@xa}}{\pgfshade@cos}{\pgfpointorigin}%
      \pgf@shade@extra@transform%
    }%
  \fi%
}


% Additionally transform masks
%
...
% Example:
%
% \pgfsetadditionalfadingtransform{\pgftransformrotate{30}}

\def\pgfsetadditionalfadingtransform#1{\def\pgf@mask@extra@transform{#1}}
\let\pgf@mask@extra@transform=\pgfutil@empty
