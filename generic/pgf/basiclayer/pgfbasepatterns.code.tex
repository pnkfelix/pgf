\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/basiclayer/Attic/pgfbasepatterns.code.tex,v 1.1 2006/03/01 23:57:34 tantau Exp $
...
\def\pgfdeclarepatternuncolored#1#2#3#4#5{%
  ...
  {%
    ...
    \expandafter\global\expandafter\let\csname pgf@pattern@uncolored@#1\endcsname=\@empty%
    \xdef\pgf@marshal{\noexpand\pgfsys@declarepatternuncolored
      {\csname pgf@pattern@name@#1\endcsname}
      {\the\pgf@xa}{\the\pgf@ya}{\the\pgf@xb}{\the\pgf@yb}{\the\pgf@xc}{\the\pgf@yc}{\pgf@pattern@code}}%
  }%
...
\def\pgfifpatterniscolored#1#2#3{%
  \@ifundefined{pgf@pattern@uncolored@#1}{#2}{#3}%
}



% Set an uncolored pattern as fill color
%
% #1 = Name of the pattern
% #2 = Color to be used for the pattern
%
% Description:
%
...
\def\pgfsetfillpatternuncolored#1#2{%
  \@ifundefined{pgf@pattern@name@#1}{%
    \PackageError{pgfbasepatterns}{Undefined pattern `#1'}{}}
  {%
    \@ifundefined{pgf@pattern@uncolored@#1}
    {%
      \PackageError{pgfbasepatterns}{Pattern `#1' is not an uncolored
        pattern}{}%
    }%
    {%
...
\def\pgfdeclarepatterncolored#1#2#3#4#5{%
  \@ifundefined{pgf@pattern@name@#1}{%
  \pgfsysprotocol@getcurrentprotocol\pgf@pattern@temp%
  {%
    \pgfinterruptpath%
    ...
    \xdef\pgf@pattern@number{\the\@tempcnta}%
    \expandafter\xdef\csname pgf@pattern@name@#1\endcsname{\the\@tempcnta}%
    \xdef\pgf@marshal{\noexpand\pgfsys@declarepatterncolored
      {\csname pgf@pattern@name@#1\endcsname}
      {\the\pgf@xa}{\the\pgf@ya}{\the\pgf@xb}{\the\pgf@yb}{\the\pgf@xc}{\the\pgf@yc}{\pgf@pattern@code}}%
  }%
  \pgfsysprotocol@setcurrentprotocol\pgf@pattern@temp%
...
% Set a  colored pattern as fill color
%
% #1 = Name of the pattern
%
% Description:
...
% Example:
%
% \pgfsetfillpatterncolored{red stars}

\def\pgfsetfillpatterncolored#1{%
  \@ifundefined{pgf@pattern@name@#1}{%
    \PackageError{pgfbasepatterns}{Undefined pattern `#1'}{}}
  {%
    \@ifundefined{pgf@pattern@uncolored@#1}
    ...
    {%
      \PackageError{pgfbasepatterns}{Pattern `#1' is not a colored
        pattern}{}%
    }%
  }%
}
