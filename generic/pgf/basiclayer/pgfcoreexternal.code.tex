...
\ProvidesFileRCS $Header: /cvsroot/pgf/pgf/generic/pgf/basiclayer/pgfcoreexternal.code.tex,v 1.5 2010/06/30 10:25:08 ludewich Exp $
...
\def\pgfincludeexternalgraphics#1{%
	...
	\ifdim\pgfexternalhskip=0sp
	\else
		\hskip\pgfexternalhskip\relax
	\fi
...
% '#1' and \pgfexternalhskip to be the 'hbaseline' shift.
%
%
...
\def\pgfexternalreaddpth#1{%
	...
	%
	\def\pgfexternaldepth{0pt}%
	\def\pgfexternalhskip{0pt}%
...
\def\pgf@external@grab#1{%
  ...
  \fi
  \global\let\pgf@hshift@baseline=\pgfutil@empty
...
\def\pgf@externalend@storeshifts#1{%
	...
	\ifdim\pgf@hshift@baseline=0sp \else
		\immediate\write#1{\noexpand\tikzexternal@restore{\noexpand\def\noexpand\pgfexternalhskip{\pgf@hshift@baseline}}}%
	\fi
}
% REMARK:
%   this method is also invoked from within the tikz external library.
...
%    \rightmargin0pt%
	...
	\fi
	\ifdim\pgf@hshift@baseline=0sp \else
		\setbox\pgfpic=\hbox{\hskip-\pgf@hshift@baseline\box\pgfpic}%
	\fi
