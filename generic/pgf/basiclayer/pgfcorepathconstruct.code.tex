...
\ProvidesFileRCS $Header: /cvsroot/pgf/pgf/generic/pgf/basiclayer/pgfcorepathconstruct.code.tex,v 1.22 2010/08/03 08:46:58 ludewich Exp $
...
\def\pgfpatharctoprecomputed#1#2#3#4#5#6#7#8{%
	...
	\ifx\pgfpatharctomaxstepsize\pgfutil@empty
		\def\pgfpath@N{1}%
	\else
		\pgf@xa=\pgf@ya% compute N = floor((gamma_1 - gamma_0) / max) +1
		\divide\pgf@xa by\pgfpatharctomaxstepsize\relax
		\afterassignment\pgfutil@gobble@until@relax
		\c@pgf@counta=\the\pgf@xa\relax
	...
	\fi
	%
	\edef\pgfpath@h{\pgf@sys@tonumber\pgf@ya}%
	%
%\message{pgfpathellipse: using N =\pgfpath@N\space spline points y0 = #2, y0+i*h, yN=#3, i=1,...,(\pgfpath@N-1), with h=\pgfpath@h\space mesh width.}%
	%
	%
	\pgf@ya=0.116355283466289\pgf@ya % pi/27
	\divide\pgf@ya by20
	\pgf@xa=#7\pgf@ya
	...
	\ifnum\c@pgf@countd<\pgfpath@N\relax
		%
		\pgf@xa=#2pt % compute \pgf@xa = y_0 + i*h
		\pgf@xb=\pgfpath@h pt
...
%\message{angle \pgfpath@N: #3...}%
	\pgfpatharcofellipse@{#4}%
	\endgroup
}%
