...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/libraries/Attic/pgflibraryshapes.multipart.code.tex,v 1.8 2008/03/12 11:55:01 vibrovski Exp $
...
% /pgf/rectangle split ignore empty parts : Ignore empty parts.
...
\pgfdeclareshape{rectangle split}{%
	...
	\savedmacro\getrectanglesplitparameters{%
		...
		% unless the empty parts are ignored.
		%
		...
		\pgfutil@for\partname:={text,second,third,fourth}\do{%
			\expandafter\let\expandafter\currentbox\csname pgfnodepart\partname box\endcsname% 
			\ifnum\parts>\c@pgf@counta%
			  \pgf@y\ht\currentbox%
				\advance\pgf@y\dp\currentbox%
				\ifdim\pgf@y=0pt\relax%
					\ifpgfrectanglesplitignoreemptyparts%
						\setbox\currentbox\copy\pgf@lib@voidbox%
					\else%
						\setbox\currentbox\hbox{\vrule width0pt height\the\pgfutil@tempdima depth0pt}%
					\fi%
				\fi%
			\else%
				\setbox\currentbox\copy\pgf@lib@voidbox%
  	...
  	%
		...
		\let\lowestanchor\textanchor%
		\let\lowestbox\pgfnodeparttextbox
		%
		\ifvoid\pgfnodepartsecondbox%
			\pgfextract@process\secondanchor{\lowestanchor}%
		\else%
		...
		\fi%
		%
		\ifvoid\pgfnodepartthirdbox%
			\pgfextract@process\thirdanchor{\lowestanchor}%
		\else%
		...
		\fi%
		%
		\ifvoid\pgfnodepartfourthbox%
			\pgfextract@process\fourthanchor{\lowestanchor}%
		\else%
