...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/libraries/Attic/pgflibraryshapes.symbols.code.tex,v 1.10 2007/11/10 09:39:32 vibrovski Exp $
...
\pgfkeys{/pgf/cloud puffs/.initial=10}%

\pgfkeys{/pgf/cloud puff arc/.initial=150}%
...
\pgfdeclareshape{cloud}{%
	\savedmacro\getradii{%
		...
		%
		% x = (X'cos(a/2)-kY')/cos^2(a/2)-k^2, 
		% y = (Y'cos(a/2)-kX')/cos^2(a/2)-k^2, 
		%
		\pgf@xc\k pt\relax%
		\pgf@xc-\k\pgf@xc%
		...
		\pgfmathreciprocal@{\pgfmath@tonumber{\pgf@xc}}% cos^2(a/2)-k^2
		%
...
\pgfdeclareshape{tape}{%
	\savedmacro\gettapepoints{%
		...
		\pgfextract@process\southeastanchor{%
		...
		\pgfextract@process\southanchor{%
		...
		\pgfextract@process\eastanchor{\centerpoint\pgf@ya\pgf@y\northeastanchor\pgf@y\pgf@ya}%
		\pgfextract@process\westanchor{\centerpoint\pgf@ya\pgf@y\southwestanchor\pgf@y\pgf@ya}%
		\addtosavedmacro{\northanchor}%
...
\pgfdeclareshape{single arrow}{%
	\savedmacro\getsinglearrowpoints{%
		...
		%
		\pgfextract@process\arrowtip{%
			\pgf@x\pgf@xa%
			\advance\pgf@x\pgf@xb%
			\pgf@y0pt\relax%
		}%
		\pgfextract@process\beforearrowtip{%
			\pgf@x\pgf@xa%
			\advance\pgf@x-\pgf@xc%
			\pgf@y\pgf@ya%
			\advance\pgf@y\pgf@yc%
		}%
		\pgfextract@process\beforearrowhead{%
			\pgf@x\pgf@xa%
			\advance\pgf@x-\pgf@xc%
			\advance\pgf@x\pgfutil@tempdima%
			\pgf@y\pgf@ya%
		}%
		\pgfextract@process\afterarrowtail{%
			\pgf@x-\pgf@xa%
			\pgf@y\pgf@ya%
		}%
		%
		...
		%
		\pgfextract@process\arrowtipanchor{%
		...
		%
		\pgfextract@process\beforearrowtipanchor{%
		...
		\pgfutil@tempdima-\pgfutil@tempdima%
		\advance\pgfutil@tempdima180pt\relax%
		\pgfextract@process\beforearrowheadanchor{%
			\pgfqpointpolar{\pgfmath@tonumber{\pgfutil@tempdima}}{\the\pgf@xa}%
			\pgf@xb\pgf@x%
			\pgf@yb\pgf@y%
		...
		\pgfextract@process\afterarrowtailanchor{%
