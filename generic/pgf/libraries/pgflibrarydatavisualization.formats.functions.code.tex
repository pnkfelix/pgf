...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/libraries/Attic/pgflibrarydatavisualization.formats.functions.code.tex,v 1.1 2008/06/09 10:29:49 tantau Exp $
...
\pgfdeclaredataformat{TeX code}
{ % no catcode code
}
{ % Startup
  % Init collectors
  \let\value=\relax% avoid expansion
  \pgfkeysgetvalue\pgf@lib@dv@collector{/pgf/data visualization/vars}
}
{% Line arguments
  #1%
}
...
\pgfkeys{/pgf/data visualization/funcs.vars={},/pgf/data visualization/funcs.initial=}
...
\def\pgf@lib@dv@func func #1=#2;{%
  \expandafter\def\expandafter\pgf@lib@dv@funcs\expandafter{\pgf@lib@dv@funcs\pgf@lib@dv@eval@func{#1}{#2}}
...
\def\pgf@lib@dv@var var #1=[#2:#3]{%
  \pgfutil@ifnextchar s{%
    ...
    \expandafter\def\expandafter\pgf@lib@dv@vars\expandafter{\pgf@lib@dv@vars\pgf@lib@dv@step@var@plain{#1}{#2}{#3}}%
...
\def\pgf@lib@dv@step#1#2#3tep#4;{%
  \expandafter\def\expandafter\pgf@lib@dv@vars\expandafter{\pgf@lib@dv@vars\pgf@lib@dv@step@var@steps{#1}{#2}{#3}{#4}}%
  \pgf@lib@dv@func@parse%
}
\def\pgf@lib@dv@samples#1#2#3amples#4;{%
  \expandafter\def\expandafter\pgf@lib@dv@vars\expandafter{\pgf@lib@dv@vars\pgf@lib@dv@step@var@samples{#1}{#2}{#3}{#4}}%
  \pgf@lib@dv@func@parse%
}
