...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/libraries/Attic/pgflibraryshapes.geometric.code.tex,v 1.13 2007/08/09 11:01:59 vibrovski Exp $
...
% Keys for border construction/rotation:
%
% /pgf/shape border uses incirle : Use the incircle to create the shape border.
% /pgf/shape border rotate       : The rotation of the shape border.
%

\newif\ifpgfshapeborderusesincircle
\pgfkeys{/pgf/shape border uses incircle/.is if=pgfshapeborderusesincircle}
\pgfkeys{/pgf/shape border uses incircle=false}

\pgfkeys{/pgf/shape border rotate/.value required}
\pgfkeys{/pgf/shape border rotate/.code={%
    \pgfmathparse{#1}%
    \pgfmathmod@{\pgfmathresult}{360}%
    \ifdim\pgfmathresult pt<0pt\relax%
      \pgfmathadd@{\pgfmathresult}{360}%
    \fi%
    \pgfkeyslet{/pgf/shape border rotate}{\pgfmathresult}%
  }%
}
\pgfkeys{/pgf/shape border rotate=0}
...
\newif\ifpgf@sh@starouterradiususesratio
...
\pgfkeys{/pgf/star points/.value required}
\pgfkeys{/pgf/star points/.code={%
		\pgfmathtruncatemacro\pgfmathresult{#1}%
		\pgfkeyslet{/pgf/star points}{\pgfmathresult}%	
	}%
}
\pgfkeys{/pgf/star points=5}

\pgfkeys{/pgf/star point height/.value required}
\pgfkeys{/pgf/star point height/.code={%
		\pgfmathparse{#1}%
		\edef\pgfmathresult{\pgfmathresult pt}%
		\pgfkeyslet{/pgf/star point height}{\pgfmathresult}%
		\pgf@sh@starouterradiususesratiofalse%	
	}%
}
\pgfkeys{/pgf/star point height=.5cm}

\pgfkeys{/pgf/star point ratio/.value required}
\pgfkeys{/pgf/star point ratio/.code={%
		\pgfmathparse{#1}%
		\pgfkeyslet{/pgf/star point ratio}{\pgfmathresult}%	
		\pgf@sh@starouterradiususesratiotrue%	
	}%
}
\pgfkeys{/pgf/star point ratio=1.5}
...
\pgfdeclareshape{star}{
	...
	\savedmacro\calculateradii{%
		...
		%
		\ifpgf@sh@starouterradiususesratio
			\expandafter\pgf@x\pgfkeysvalueof{/pgf/star point ratio}\pgf@x%
		\else%
			\advance\pgf@x\pgfkeysvalueof{/pgf/star point height}\relax%
		\fi%
		\pgf@xc\pgf@x%
		%
	   ...
	   \ifdim\pgf@x>\pgf@xc%
	   	...
	   	\ifpgf@sh@starouterradiususesratio%
			...
			\else%
				...
				%
				\pgf@xa\pgf@x\relax%
				\advance\pgf@xa-\pgfkeysvalueof{/pgf/star point height}\relax%
				\edef\innerradius{\the\pgf@xa}%
			\fi%
		\fi%
		\edef\outerradius{\the\pgf@x}%
...
\pgfkeys{/pgf/regular polygon sides/.value required}
\pgfkeys{/pgf/regular polygon sides/.code={%
		\pgfmathtruncatemacro\pgfmathresult{#1}%
		\pgfkeyslet{/pgf/regular polygon sides}{\pgfmathresult}%
	}%
}
\pgfkeys{/pgf/regular polygon sides=5}
...
\pgfdeclareshape{regular polygon}{
	\savedmacro\sides{%
		\pgfkeysgetvalue{/pgf/regular polygon sides}{\sides}%
	}
	\savedmacro\anglestep{%
	...
	\savedmacro\startangle{%
		\expandafter\ifodd\pgfkeysvalueof{/pgf/regular polygon sides}%
...
%
% /pgf/trapezium left extension
% /pgf/trapezium right extension
%

\pgfkeys{/pgf/trapezium left extension/.value required}
\pgfkeys{/pgf/trapezium left extension/.code={%
		\pgfmathparse{#1}%
		\edef\pgfmathresult{\pgfmathresult pt}%
		\pgfkeyslet{/pgf/trapezium left extension}{\pgfmathresult}%
	}%
}
\pgfkeys{/pgf/trapezium left extension=.333cm}

\pgfkeys{/pgf/trapezium right extension/.value required}
\pgfkeys{/pgf/trapezium right extension/.code={%
		\pgfmathparse{#1}%
		\edef\pgfmathresult{\pgfmathresult pt}%
		\pgfkeyslet{/pgf/trapezium right extension}{\pgfmathresult}%
	}%
}
\pgfkeys{/pgf/trapezium right extension=.333cm}

\pgfkeys{/pgf/trapezium extension/.value required}
\pgfkeys{/pgf/trapezium extension/.code={%
		\pgfkeys{/pgf/trapezium left extension=#1}
		\pgfkeys{/pgf/trapezium right extension=#1}
	}%
}
...
\pgfdeclareshape{trapezium}{
	\savedmacro\installtrapeziumparameters{%
		...
		\pgfmathsetlength\pgf@xa{+\pgfkeysvalueof{/pgf/trapezium left extension}}%
		\edef\leftextension{\the\pgf@xa}%
		\ifdim\pgf@xa<0pt\relax%
			\pgf@xa-\pgf@xa%
		\fi%
		\pgfmathsetlength\pgf@xb{+\pgfkeysvalueof{/pgf/trapezium right extension}}%
		...
		%
		\pgfmathsetlength\pgf@yc{\pgfkeysvalueof{/pgf/minimum width}}%
		\ifdim\pgf@xb<\pgf@yc%
			\pgf@xb\pgf@yc%
			\pgf@xc\pgf@xb%
...
%
% /pgf/isosceles trangle apex angle

\pgfkeys{/pgf/isosceles triangle apex angle/.value required}
\pgfkeys{/pgf/isosceles triangle apex angle/.code={%
		\pgfmathparse{#1}%
		\pgfkeyslet{/pgf/isosceles triangle apex angle}{\pgfmathresult}%
	}%
}
\pgfkeys{/pgf/isosceles triangle apex angle=45}
...
\pgfdeclareshape{isosceles triangle}{
	\savedmacro\installtriangleparameters{%
		...
		%
		\pgfmathdivide@{\pgfkeysvalueof{/pgf/isosceles triangle apex angle}}{2}%
		...
		\addtosavedmacro{\rotate}%
		\addtosavedmacro{\outersep}%
		\addtosavedmacro{\lowerhorizontalmiter}%
		\addtosavedmacro{\apexmiter}%
		\addtosavedmacro{\apexpoint}%
		\addtosavedmacro{\lowerleftpoint}%
		\addtosavedmacro{\lowerrightpoint}%
		\addtosavedmacro{\apex}%
		\addtosavedmacro{\lowerleft}%
		\addtosavedmacro{\lowerright}%
		%
		\addtosavedmacro{\angletoapex}%
		\addtosavedmacro{\angletolowerleft}%
		\addtosavedmacro{\angletolowerright}%
		%
		\addtosavedmacro{\baseangletoapex}%
		\addtosavedmacro{\baseangletolowerleft}%
		\addtosavedmacro{\baseangletolowerright}%
		%
		\addtosavedmacro{\midangletoapex}%
		\addtosavedmacro{\midangletolowerleft}%
		\addtosavedmacro{\midangletolowerright}%
		%
		\addtosavedmacro{\externalradius}%
...
%
%
\pgfkeys{/pgf/kite upper vertex angle/.value required}
\pgfkeys{/pgf/kite upper vertex angle/.code={%
		\pgfmathparse{#1}%
		\pgfkeyslet{/pgf/kite upper vertex angle}{\pgfmathresult}%
	}%
}
\pgfkeys{/pgf/kite upper vertex angle=120}

\pgfkeys{/pgf/kite lower vertex angle/.value required}
\pgfkeys{/pgf/kite lower vertex angle/.code={%
		\pgfmathparse{#1}%
		\pgfkeyslet{/pgf/kite lower vertex angle}{\pgfmathresult}%
	}%
}
\pgfkeys{/pgf/kite lower vertex angle=60}

\pgfkeys{/pgf/kite vertex angles/.value required}
\pgfkeys{/pgf/kite vertex angles/.code={%
		\pgfutil@in@,{#1}%
		\ifpgfutil@in@%
			\def\pgf@marshall##1,##2\@@{%
				\pgfkeys{/pgf/kite upper vertex angle=##1}%
				\pgfkeys{/pgf/kite lower vertex angle=##2}%
			}%
			\expandafter\pgf@marshall#1\@@%
		\else%
			\pgfkeys{/pgf/kite upper vertex angle=#1}%
...
\pgfkeys{/pgf/dart tip angle/.value required}
\pgfkeys{/pgf/dart tip angle/.code={%
		\pgfmathparse{#1}%
		\pgfkeyslet{/pgf/dart tip angle}{\pgfmathresult}%
	}%
}
\pgfkeys{/pgf/dart tip angle=45}

\pgfkeys{/pgf/dart tail angle/.value required}
\pgfkeys{/pgf/dart tail angle/.code={%
		\pgfmathparse{#1}%
		\pgfkeyslet{/pgf/dart tail angle}{\pgfmathresult}%
	}%
}
\pgfkeys{/pgf/dart tail angle=135}
...
\pgfkeys{/pgf/circular sector angle/.value required}
\pgfkeys{/pgf/circular sector angle/.code={%
    \pgfmathparse{#1}%
    \pgfkeyslet{/pgf/circular sector angle}{\pgfmathresult}%
  }%
}
\pgfkeys{/pgf/circular sector angle=60}
...
\pgfdeclareshape{circular sector}{%
	\savedmacro\installcircularsectorparameters{%
		...
		%
		\pgfsavepgf@process\sectorcenter{%
			\centerpoint%
			\advance\pgf@x\centeroffset%
		}%
		\pgfsavepgf@process\arcstart{%
		...
		%
		\pgfsavepgf@process\sectorcenterborder{%
			\sectorcenter%
