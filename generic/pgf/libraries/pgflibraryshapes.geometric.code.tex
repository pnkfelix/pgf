...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/libraries/Attic/pgflibraryshapes.geometric.code.tex,v 1.18 2007/10/15 15:03:06 vibrovski Exp $
...
\pgfkeys{/pgf/star point ratio/.code={%
		\pgfkeyssetvalue{/pgf/star point ratio}{#1}%
...
%
\pgfkeys{/pgf/regular polygon sides/.initial=5}
...
\pgfkeys{/pgf/trapezium left angle/.initial=60}

\pgfkeys{/pgf/trapezium right angle/.value required}
\pgfkeys{/pgf/trapezium right angle/.initial=60}

\pgfkeys{/pgf/trapezium angle/.value required}
\pgfkeys{/pgf/trapezium angle/.code={%
		\pgfkeys{/pgf/trapezium left angle=#1}
		\pgfkeys{/pgf/trapezium right angle=#1}
	}%
}
...
\pgfkeys{/pgf/isosceles triangle apex angle/.initial=45}


% Shape isosceles triangle
%
\pgfdeclareshape{isosceles triangle}{%
	\savedmacro\getisoscelestrianglepoints{%
		%
		...
		\else%
			...
			\pgfmathreciprocal@{\pgfmathresult}%
			\pgf@yb\pgfmathresult\pgf@xa%
			\advance\pgf@yb2.0\pgf@ya%
		\fi%
		\addtosavedmacro\rotate%		
		%
		\pgfmathsetlength\pgf@xc{\pgfkeysvalueof{/pgf/minimum width}}%
		\ifdim\pgf@xb<.5\pgf@xc%
			\pgf@xc.5\pgf@xc%
			\ifdim\pgf@xb=0pt\relax%
				\pgf@xb\pgf@xc%
				\pgf@ya0pt\relax%
				\pgfmathcot@{\halfapexangle}%
				\pgf@yb\pgfmathresult\pgf@xb%
			\else%
				\pgfmathdivide@{\pgfmath@tonumber{\pgf@xc}}{\pgfmath@tonumber{\pgf@xb}}%
				\pgf@xb\pgf@xc%
				\pgf@ya\pgfmathresult\pgf@ya%
				\pgf@yb\pgfmathresult\pgf@yb%
			\fi%
		...
		\ifdim\pgf@yb<\pgf@yc%
			\ifdim\pgf@yb=0pt\relax%
				\pgf@yb\pgf@yc%
				\pgf@ya.333333\pgf@yb% Arbitrary choice.
				\pgfmathtan@{\halfapexangle}%
				\pgf@xb\pgfmathresult\pgf@yb%
				\pgf@yb.666667\pgf@yb% Arbitrary choice.
			\else%
		...
		%
		\pgfmathsetlength\pgfutil@tempdima{\pgfkeysvalueof{/pgf/outer xsep}}%
		...
		\pgfextract@process\apexanchor{%
			\apex%
			\pgfmathcosec@{\halfapexangle}%
			\advance\pgf@x\pgfmathresult\pgfutil@tempdima%
		}%
		\pgfmathsubtract@{90}{\halfapexangle}%
		\pgfmathdivide@{\pgfmathresult}{2}%
		\let\lowerangle\pgfmathresult%
		\pgfmathcosec@{\pgfmathresult}%
		\pgfutil@tempdimb\pgfmathresult\pgfutil@tempdima%
		%
		\pgfextract@process\lowerleftanchor{%
			\pgfmathadd@{\lowerangle}{90}%
			\let\angle\pgfmathresult%
			\pgfpointadd{\lowerleft}{\pgfqpointpolar{\angle}{\the\pgfutil@tempdimb}}%
		}%
		\pgfextract@process\lowerrightanchor{%
			\pgfmathsubtract@{270}{\lowerangle}%
			\let\angle\pgfmathresult%
			\pgfpointadd{\lowerright}{\pgfqpointpolar{\angle}{\the\pgfutil@tempdimb}}%
		}%
		...
		%
		\ifdim\pgf@yb>2.0\pgf@xb%
			\advance\pgf@yb\pgfutil@tempdima%
			\edef\externalradius{\the\pgf@yb}%
		\else%
			\advance\pgf@xb\pgfutil@tempdima%
			\edef\externalradius{\the\pgf@xb}%
		\fi%
		\addtosavedmacro\externalradius%			
	}
	\savedanchor\centerpoint{%
	...
	\anchor{apex}{%
		\getisoscelestrianglepoints%
		\pgfmathrotatepointaround{\pgfpointadd{\apexanchor}{\centerpoint}}{\centerpoint}{\rotate}%
	}%
	\anchor{left corner}{%
		\getisoscelestrianglepoints%
		\pgfmathrotatepointaround{\pgfpointadd{\lowerleftanchor}{\centerpoint}}{\centerpoint}{\rotate}%
	}
	\anchor{right corner}{%
		\getisoscelestrianglepoints%
		\pgfmathrotatepointaround{\pgfpointadd{\lowerrightanchor}{\centerpoint}}{\centerpoint}{\rotate}%
	}
	\anchor{left side}{%
		\getisoscelestrianglepoints%
		\pgfmathrotatepointaround{%
...
\pgfkeys{/pgf/kite upper vertex angle/.initial=120}

\pgfkeys{/pgf/kite lower vertex angle/.value required}
\pgfkeys{/pgf/kite lower vertex angle/.initial=60}
...
\pgfkeys{/pgf/dart tip angle/.initial=45}

\pgfkeys{/pgf/dart tail angle/.value required}
\pgfkeys{/pgf/dart tail angle/.initial=135}
...
\pgfdeclareshape{cylinder}{%
	\savedmacro\getcylinderpoints{%
		...
		\pgf@xa\pgf@x%
		\pgf@ya\pgf@y%
		\edef\cylinderaspect{\pgfkeysvalueof{/pgf/shape aspect}}%
		\pgfutil@tempdima\cylinderaspect\pgf@ya%
