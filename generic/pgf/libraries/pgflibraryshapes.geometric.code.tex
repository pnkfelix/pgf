...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/libraries/Attic/pgflibraryshapes.geometric.code.tex,v 1.15 2007/08/20 12:48:39 vibrovski Exp $
...
%
% /pgf/shape aspect         : recommended width/height quotient.
% /pgf/shape aspect inverse : recommended height/width quotient.
%
\pgfkeys{/pgf/shape aspect/.code={%
		\pgfmathparse{#1}%
		\pgfkeyslet{/pgf/shape aspect}{\pgfmathresult}%
		\pgfmathreciprocal@{\pgfmathresult}%
		\pgfkeyslet{/pgf/shape aspect inverse}{\pgfmathresult}%
	}%
}
\pgfkeys{/pgf/shape aspect inverse/.code={%
		\pgfmathparse{#1}%
		\pgfkeyslet{/pgf/shape aspect inverse}{\pgfmathresult}%
		\pgfmathreciprocal@{\pgfmathresult}%
		\pgfkeyslet{/pgf/shape aspect}{\pgfmathresult}%
...
\pgfdeclareshape{diamond}
{
  \savedanchor\outernortheast{%
    ...
    %
    \pgf@x=\pgf@xa%
    \advance\pgf@x by\pgfkeysvalueof{/pgf/shape aspect}\pgf@ya%
    \pgf@y=\pgfkeysvalueof{/pgf/shape aspect inverse}\pgf@xa%
    \advance\pgf@y by\pgf@ya%
    %
...
\pgfkeys{/pgf/trapezium left angle/.value required}
\pgfkeys{/pgf/trapezium left angle/.code={%
		\pgfmathparse{#1}%
	  \pgfmathmod@{\pgfmathresult}{360}%
    \ifdim\pgfmathresult pt<0pt\relax%
      \pgfmathadd@{\pgfmathresult}{360}%
    \fi%
    \pgfkeyslet{/pgf/trapezium left angle}{\pgfmathresult}%
  }%
}
\pgfkeys{/pgf/trapezium left angle=60}

\pgfkeys{/pgf/trapezium right angle/.value required}
\pgfkeys{/pgf/trapezium right angle/.code={%
		\pgfmathparse{#1}%
	  \pgfmathmod@{\pgfmathresult}{360}%
    \ifdim\pgfmathresult pt<0pt\relax%
      \pgfmathadd@{\pgfmathresult}{360}%
    \fi%
    \pgfkeyslet{/pgf/trapezium right angle}{\pgfmathresult}%
  }%
}
\pgfkeys{/pgf/trapezium right angle=60}

\pgfkeys{/pgf/trapezium angle/.value required}
\pgfkeys{/pgf/trapezium angle/.code={%
...
\pgfdeclareshape{trapezium}{
	\savedmacro\installtrapeziumparameters{%
		...
		\else%
			...
			\fi%
		\fi%
		\pgfkeysgetvalue{/pgf/trapezium left angle}{\leftangle}%
		\pgfmathcot@{\leftangle}%
		\pgf@xa2\pgf@y%
		\pgf@xa\pgfmathresult\pgf@xa%
		\pgfkeysgetvalue{/pgf/trapezium right angle}{\rightangle}%
