...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/libraries/Attic/pgflibrarydatavisualization.code.tex,v 1.2 2008/03/03 10:43:51 tantau Exp $
...
\pgfooclass{candle stick visualizer}
{
  \method candle stick visualizer(#1) {
    ...
    \pgfooset{use path rise}{\pgfsetfillcolor{white}\pgfusepath{fill,stroke}}
    \pgfooset{use path fall}{\pgfsetfillcolor{black}\pgfusepath{fill,stroke}}
    \pgfooset{use path wick}{\pgfusepath{stroke}}
  }
  ...
  \method set stick width(#1) {
    ...
    \pgfkeyssetvalue{/data point/\pgfoovalue{attribute}}{10}
    \pgfcanvaspositionofvirtualdatapoint%
  ...
  \method visualize data point() {
    {
      ...
      \else%
        \begin{pgfscope}
          ...
          \pgfoovalue{use path wick}
          \pgfpathmoveto{\pgfpointadd{\pgf@dv@lib@a}{\pgfoovalue{offset}}}
          \pgfpathlineto{\pgfpointadd{\pgf@dv@lib@b}{\pgfoovalue{offset}}}
          \pgfpathlineto{\pgfpointadd{\pgf@dv@lib@b}{\pgfpointscale{-1}{\pgfoovalue{offset}}}}
          \pgfpathlineto{\pgfpointadd{\pgf@dv@lib@a}{\pgfpointscale{-1}{\pgfoovalue{offset}}}}
          \pgfpathclose
          \ifpgf@dv@lib@rise
            \pgfoovalue{use path rise}
          \else
            \pgfoovalue{use path fall}
          \fi
        \end{pgfscope}
      \fi
    }
