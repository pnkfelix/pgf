...
\ProvidesFileRCS $Header: /cvsroot/pgf/pgf/generic/pgf/modules/pgfmoduleplot.code.tex,v 1.3 2009/09/15 07:49:23 ludewich Exp $
...

{
  \catcode`\%=12
  \catcode`\"=12
  \xdef\pgf@gnuplot@head{set terminal table; set output \noexpand\pgf@plottablefile@quoted; set format "%.5f"}
}
...
\def\pgf@plotgnuplot[#1]#2{%
  ...
  \ifpgf@resample@plot%  
    ...
    \immediate\write\pgf@plotwrite{\pgf@gnuplot@head}%
