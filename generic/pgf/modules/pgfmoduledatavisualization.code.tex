...
\ProvidesFileRCS $Header: /cvsroot/pgf/pgf/generic/pgf/modules/pgfmoduledatavisualization.code.tex,v 1.4 2008/03/12 21:40:41 tantau Exp $
...
% (locally for the current group) all listeners.

\def\pgfnewdatavisualization{%
  \pgfoonewsignal{prepare data point}%
  \pgfoonewsignal{locate data point}%
  \pgfoonewsignal{transform data point}%
  \pgfoonewsignal{visualize data point}%
  \pgfoonewsignal{finish data point}%
  \pgfoonewsignal{begin data set}%
  \pgfoonewsignal{end data set}%
  \pgfoonewsignal{begin data cluster}%
  \pgfoonewsignal{end data cluster}%
}      
...
\def\pgfdatapoint{%
  ...
  \pgfooemit{visualize data point}%
  \pgfooemit{finish data point}%
}
...
% lets listening objects "locate" the object by setting up
...
\def\pgf@canvaspositionofdatapoint#1{%
  {
    \pgftransformreset
    #1%
    \pgfooemit{locate data point}
    \pgfooemit{transform data point}%
...
\def\pgfdataset{\pgfooemit{begin data set}\pgfooemit{begin data cluster}}
\def\endpgfdataset{\pgfooemit{end data cluster}\pgfooemit{end data set}}
...
\def\pgfdatacluster{\pgfooemit{begin data cluster}}
\def\endpgfdatacluster{\pgfooemit{end data cluster}}
...
\pgfooclass{line transformer}
{
  ...
  \method default connects() {
    \pgfoothis.connect(transform data point)
  }
...
\pgfooclass{attribute locator}
{
  ...
  \method default connects() {
    \pgfoothis.connect(locate data point)
  }
...
\pgfooclass{count up}
{
  ...
  \method default connects() {
    \pgfoothis.connect(prepare data point)
  }
...
\pgfooclass{plot mark visualizer}
{
  ...
  \method default connects() {
    \pgfoothis.connect(visualize data point)
  }
...
\pgfooclass{plot handler visualizer}
{
  ...
  \method default connects() {
    \pgfoothis.connect(visualize data point)
    \pgfoothis.connect(end data cluster)
  }
