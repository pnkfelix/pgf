...
\ProvidesFileRCS $Header: /cvsroot/pgf/pgf/generic/pgf/modules/pgfmoduledatavisualization.code.tex,v 1.32 2010/05/26 20:11:33 tantau Exp $
...
\pgfooclass{plot handler visualizer}
{
  ...
  %
  % When you create a plot handler visualizer, you must give it a "name". 
  % Only those data points where the current value of the
  % key /data point/visualizer equals this name will become part of the
  % plot. This makes it easy to specify multiple plots. 
  %
  % The keys /data point/<name>/execute at begin stream and /data
  % point/<name>/execute at end stream, where <name> is the name of
  ...
  \method protocol() {
    ...
    \pgfooget{name}\pgf@dv@name%
    \ifx\pgf@dv@name\pgf@dv@visualizer%
  ...
  \method render() {
    \pgfscope
      \pgfkeysvalueof{/data point/\pgfoovalueof{name}/execute at begin stream}
      \pgfoovalueof{handler}
      \pgfplotstreamstart
      \pgfoovalueof{positions}
      \pgfoovalueof{cache}
      \pgfplotstreamend
      \pgfkeysvalueof{/data point/\pgfoovalueof{name}/execute at end stream}
    \endpgfscope
