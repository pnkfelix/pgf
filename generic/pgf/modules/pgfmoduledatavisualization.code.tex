...
\ProvidesFileRCS $Header: /cvsroot/pgf/pgf/generic/pgf/modules/pgfmoduledatavisualization.code.tex,v 1.29 2009/11/29 16:47:59 ludewich Exp $
...
\pgfooclass{interval mapper}
{
  ...
  \method map() {
    ...
    \else%
      ...
      \else%
        ...
        \fi%
      \fi%
      \pgfkeysgetvalue{/data point/\pgfoovalueof{in}/min}\pgfmathresult%
      \ifx\pgfmathresult\relax\else%
        \ifx\pgfmathresult\pgfutil@empty\else%
          \pgf@dv@mapper@trans{/min}
        \fi
      \fi
      \pgfkeysgetvalue{/data point/\pgfoovalueof{in}/max}\pgfmathresult%
      \ifx\pgfmathresult\relax\else%
        \ifx\pgfmathresult\pgfutil@empty\else%
  ...
  \def\pgf@dv@mapper@trans#1{
    ...
    \pgfkeysgetvalue{/data point/\pgfoovalueof{out}#1/offset}\pgf@dv@offset%
    \ifx\pgf@dv@offset\relax\else%
      \ifx\pgf@dv@offset\pgfutil@empty\else%
        \pgfmathadd{\pgfmathresult}{\pgf@dv@offset}
      \fi
    \fi
    \pgfkeyslet{/data point/\pgfoovalueof{out}#1}\pgfmathresult%
  }
}
...
\pgfooclass{scaling mapper}
{
  ...
  % values of the in interval. Thus, a value of
  % ".5*\pgfdvmin+.5*\pgfdvmax" yields the middle of the interval.
  %
  ...
  \method set in to(#1) {
    ...
    \else
      ...
      \fi
    \fi
    \pgfkeyslet{/data point/\pgfoovalueof{in}}\pgfmathresult
  }
  ...
  \def\pgf@dv@range@surv{%
    ...
    \else%
      ...
      \else%
        ...
        \else%
          \pgfmathlessthan{\pgf@dv@val}{\pgfdvmin}%
        ...
        \else%
          \pgfmathgreaterthan{\pgf@dv@val}{\pgfdvmax}%
          \ifdim\pgfmathresult pt=1.0pt%
            \let\pgfdvmax\pgf@dv@val%
          \fi%
        \fi%
...
\pgfooclass{plot handler visualizer}
{
  ...
  %
  % This visualizer uses a plot handler to visualize data points.
  ...
  %
  % A group of data points is given by a list of consecutive data
  % points. Both membership and the end of this list are determined by
  % a control attribute. When the control attribute is empty, the data
  % point is ignored. When the control attribute's value changes, the
  % old data group ends and a new data group starts. In order to
  % render the data group, the control's subkey "use path" is used
  % with the value it had for the first data point of the group.
  % 
  ...
  \attribute use path;
  % The action to be taken on the path
  ...
  % Slot
  \method protocol() {
    \pgfkeysgetvalue{/data point/\pgfoovalueof{control}}\pgf@dv@control%
    \ifx\pgf@dv@control\relax%
    \else%
      \ifx\pgf@dv@control\pgfutil@empty%
      \else%
        \pgfooget{control value}\pgf@dv@control@prev%
        \ifx\pgf@dv@control@prev\pgf@dv@control%
        \else%
          \ifx\pgf@dv@control@prev\pgfutil@empty%
            % first
          \else
            \pgfoothis.render()% flush out this group
          \fi%
          \pgfkeysgetvalue{/data point/\pgfoovalueof{control}/use path}\pgf@temp
          \pgfoolet{use path}\pgf@temp
          \pgfoolet{control value}\pgf@dv@control%
        \fi%
        % Cache it
