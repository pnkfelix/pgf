...
\ProvidesFileRCS $Header: /cvsroot/pgf/pgf/generic/pgf/modules/pgfmoduledatavisualization.code.tex,v 1.33 2010/05/31 10:11:22 tantau Exp $
...
\pgfooclass{label visualizer}
{
  ...
  \attribute name;
  % Name of the to-be-monitored visualizer
  ...
  \method test() {
    ...
    \pgfooget{name}\pgf@dv@name%
    \ifx\pgf@dv@name\pgf@dv@visualizer%
     % Do sth only for the correct visualizer
  ...
  \method phase(#1) {%
    \ifx#1\pgfdvendvisualization%
      \scope
        \pgftransformshift{\pgfoovalueof{canvas position}}
        \pgfoovalueof{code}
      \endscope
    \fi%
  }
