...
\ProvidesFileRCS $Header: /cvsroot/pgf/pgf/generic/pgf/modules/pgfmoduledatavisualization.code.tex,v 1.14 2008/06/15 20:37:57 tantau Exp $
...
\pgfooclass{data visualization}
{
  ...
  % visualization. 
  %
  % A bit contrary to normal oo-programming, if there are two objects
  % of this class, the second object will completely hide the first
  % one. This means that you cannot create two objects of this class
  % and then interleave calls to the two objects. However, you *can*
  % create a data visualization object and then, inside a nested
  % scope, create another such object, use it, and after the end of
  % the nested scope, return to using the first object.
  %
  % When a data visualization object is created, a whole bunch of
  % signal objects are also created. However, these are *not*
  % attributes of the data visualization object. Rather, they have
  % dedicated names of the type \pgfsignalxxxx.
  %
  % To use a data visualization object, you should (possibly
  % repeatedly) call the macro \pgfdata, which will store the data
  % in a variable that is local to the current scope. You should also
  ...
  % correct ranges. You may then create furhter objects based on this
  % data. Then, you should cann the "visualize" method, which will
  ...
  \method data visualization() {
    ...
    %
    \let\pgf@dataset@collection=\pgfutil@empty% this will store the data
  }
  ...
  \method before survey(#1) {
    \pgfooget{before survey}\pgf@temp
    \expandafter\def\expandafter\pgf@temp\expandafter{\pgf@temp#1}
    \pgfoolet{before survey}\pgf@temp
  }

  % Method
  \method after survey(#1) {
    \pgfooget{after survey}\pgf@temp
    \def\pgf@@temp{#1}%
    \expandafter\expandafter\expandafter\def%
    \expandafter\expandafter\expandafter\pgf@temp%
    \expandafter\expandafter\expandafter{\expandafter\pgf@@temp\pgf@temp}
    \pgfoolet{after survey}\pgf@temp
  }
  
  % Method
  \method before visualization(#1) {
    \pgfooget{before visualization}\pgf@temp
    \expandafter\def\expandafter\pgf@temp\expandafter{\pgf@temp#1}
    \pgfoolet{before visualization}\pgf@temp
  }

  % Method
  \method after visualization(#1) {
    \pgfooget{after visualization}\pgf@temp
    \def\pgf@@temp{#1}%
    \expandafter\expandafter\expandafter\def%
    \expandafter\expandafter\expandafter\pgf@temp%
    \expandafter\expandafter\expandafter{\expandafter\pgf@@temp\pgf@temp}
    \pgfoolet{after visualization}\pgf@temp
  }
...
\pgfooclass{range surveyor}
{
  ...
  \method default connects() {
    \pgfoothis.get handle(\pgf@dv@me)
    \pgfsignalsurveydatapoint.connect(\pgf@dv@me,survey)
  }
...
\pgfooclass{interval}
{
  ...
  \method get adjusted interval(#1,#2,#3,#4) {
    ...
    \else%
      \ifx\pgfdvmax\pgfutil@empty%
      \else%
        \pgfmathsetmarco\pgf@dv@minpa{#1}% min adjust aboslute
        \pgfmathsetmarco\pgf@dv@minpr{#2}% min adjust relative
        \pgfmathsetmarco\pgf@dv@maxpa{#3}% max adjust aboslute
        \pgfmathsetmarco\pgf@dv@maxpr{#4}% max adjust relative
        \pgfmathsubtract{\pgfdvmax}{\pgfdvmin}%
...
\pgfooclass{count}
{
  ...
  \method default connects() {
    ...
    \pgfsignalphase.connect(\pgf@dv@me,phase)
  }
