...
\ProvidesFileRCS $Header: /cvsroot/pgf/pgf/generic/pgf/modules/pgfmoduledatavisualization.code.tex,v 1.24 2008/11/13 21:10:25 tantau Exp $
...
\pgfooclass{rectangle visualizer}
{
  ...
  % #2 = second attribute 
  %
  \method rectangle visualizer(#1,#2) {
  ...
  % Slot
  \method visualize() {
    \pgfkeysgetvalue{/data point/\pgfoovalueof{attribute 1}/min}\pgf@temp%
    \ifx\pgf@temp\relax%
    \else
    \ifx\pgf@temp\pgfutil@empty%
    \else
