...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/utilities/pgfcalendar.code.tex,v 1.2 2007/02/07 17:02:40 tantau Exp $
...
% can write, say, \ifdate{mondays}{Is a Monday}{Not a Monday} to
% execute code on a Monday. 
%
% Example:
%
% \begin{tikzpicture}
...
\def\pgfcalendar#1#2#3#4#5{%
  \begingroup%
    ...
    \def\ifdate##1##2##3{%
      ...
      \pgfcalendaron{\pgfcalendarcurrentjulian}{##1}{##2}{##3}%
    ...
    \ifnum\pgfcalendarcurrentjulian<\pgfcalendarendjulianplus\relax%
      \pgfcalendarifdate{\pgfcalendarcurrentjulian}{all}{}{}% setup on macros
      % Lauch test code:
      #4%
...
\def\pgfcalendarifdate#1#2#3#4{%
  \edef\pgfcalendarifdateiso{#1}%
  \explicitly\pgfcalendar@split\pgfcalendarifdateiso\relax%
  \pgfcalendardatetojulian{\pgfcalendarifdateiso}{\pgfutil@tempcnta}
...
\def\pgfcalendar@split#1-#2-#3\relax{%
  \def\pgfcalendarifdateyear{#1}%
  \def\pgfcalendarifdatemonth{#2}%
  \def\pgfcalendarifdateday{#3}%
}
...
\define@key{pgfcalendar}{mondays}[]{\ifnum\pgfcalendarifdateweekday=0\relax\pgfcalendarmatchestrue\fi}
\define@key{pgfcalendar}{tuesdays}[]{\ifnum\pgfcalendarifdateweekday=1\relax\pgfcalendarmatchestrue\fi}
\define@key{pgfcalendar}{wednesdays}[]{\ifnum\pgfcalendarifdateweekday=2\relax\pgfcalendarmatchestrue\fi}
\define@key{pgfcalendar}{thursdays}[]{\ifnum\pgfcalendarifdateweekday=3\relax\pgfcalendarmatchestrue\fi}
\define@key{pgfcalendar}{fridays}[]{\ifnum\pgfcalendarifdateweekday=4\relax\pgfcalendarmatchestrue\fi}
\define@key{pgfcalendar}{saturdays}[]{\ifnum\pgfcalendarifdateweekday=5\relax\pgfcalendarmatchestrue\fi}

\define@key{pgfcalendar}{workdays}[]{\ifnum\pgfcalendarifdateweekday<5\relax\pgfcalendarmatchestrue\fi}
\define@key{pgfcalendar}{weekends}[]{\ifnum\pgfcalendarifdateweekday>4\relax\pgfcalendarmatchestrue\fi}

\define@key{pgfcalendar}{date}{%
  \def\pgf@@temp{#1}%
  \edef\pgf@temp{\pgfcalendarifdateyear-\pgfcalendarifdatemonth-\pgfcalendarifdateday}%
  \ifx\pgf@temp\pgf@@temp\relax%
    \pgfcalendarmatchestrue%
  \fi%
  \edef\pgf@temp{\pgfcalendarifdatemonth-\pgfcalendarifdateday}%
  \ifx\pgf@temp\pgf@@temp\relax%
    \pgfcalendarmatchestrue%
  \fi%
}
...
\def\pgfcalendarsuggestedname{\pgfcalendarprefix-\pgfcalendarifdateyear-\pgfcalendarifdatemonth-\pgfcalendarifdateday}%
