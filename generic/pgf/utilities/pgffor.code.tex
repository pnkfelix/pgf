...
\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/utilities/pgffor.code.tex,v 1.13 2008/06/23 07:24:15 vibrovski Exp $
...
\def\pgffor@var@add#1#2\pgffor@stop{%
	...
	\fi%	
	\expandafter\let\csname pgffor@var@evaluate@\string#1@trim\endcsname=\relax%
...
\def\pgffor@var@@evaluate#1#2as #3#4\pgffor@@stop{%
	...
	\expandafter\let\csname pgffor@var@evaluate\string#1@using\endcsname=\relax%
	\pgfutil@in@{(trim integers)}{#4}%
	\ifpgfutil@in@%
		\pgffor@var@@@evaluate{#1}#4\pgffor@@stop%
	\else%
		\pgffor@var@@@evaluate{#1}#4(\pgffor@@stop)\pgffor@@stop%
	\fi%
}

\def\pgffor@var@@@evaluate#1#2(#3)#4\pgffor@@stop{%
	\ifx#3\pgffor@@stop%
...
\def\pgffor@dotscharcheck#1#2\pgffor@@stop{%
  ...
  \pgffor@ifitemregister{#1}{%
  	...
  	\expandafter\chardef\expandafter\pgffor@char\expandafter=\expandafter`\the#1\relax\pgffor@@stop%
  }{%
	  \afterassignment\pgffor@gobbletil@pgffor@@stop%
