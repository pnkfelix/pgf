\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/Attic/tikz.code.tex,v 1.15 2005/08/26 13:33:12 tantau Exp $
...
\define@key{tikz}{arrange function}{\let\tikz@arrange=#1}
...
\def\tikz@tree@edge@path{(\tikzparentnode\tikzparentanchor) -- (\tikzchildnode\tikzchildanchor)}

\tikzleveldistance=15mm
\tikzsiblingdistance=15mm

\def\tikz@arrange@below@lr{%
  \pgftransformyshift{-\tikzleveldistance}
  \pgf@xa=.5\tikzsiblingdistance
  \pgftransformxshift{-\tikznumberofchildren\pgf@xa}
  \pgftransformxshift{-\pgf@xa}
  \pgftransformxshift{\tikznumberofcurrentchild\tikzsiblingdistance}
}

\let\tikz@arrange=\tikz@arrange@below@lr
...
\tikzstyle{tree edge node}=         []

\tikzstyle{arrange below rightward}=[arrange function=\tikz@arrange@below@lr]
...
\long\def\tikz@childrenA[#1]#2{%
  \begingroup%
    ...
    \ifnum\tikznumberofchildren=0\relax%
...
\long\def\tikz@countkids#1\child#2\pgf@stop{%
  \def\pgf@test{#2}%
  \ifx\pgf@test\@empty%
    \let\pgf@next=\@gobble%
  \else%
...
\def\tikz@childnode{%
  ...
  \def\tikz@child@node@options{name=\tikzparentnode-\the\tikznumberofcurrentchild}%
...
\def\tikz@childnode@continue#1{%
  ...
  \@ifnextchar t{\tikz@child@edge}{\tikz@child@edge@nonode}%
}
...
\def\tikz@child@edge@opt[#1]{%
  ...
  \@ifnextchar n{\tikz@child@edge@node}{\tikz@child@edge@nonode}%
}
\def\tikz@child@edge@node node{\@ifnextchar[{\tikz@child@edge@node@opt}{\tikz@child@edge@node@opt[]}}%}%
\def\tikz@child@edge@node@opt[#1]{%
  \def\tikz@child@edge@node@options{#1}%
  \@ifnextchar({\tikz@child@edge@node@named}{\tikz@child@edge@node@notnamed}}%}
\def\tikz@child@edge@node@named(#1){%
  \expandafter\def\expandafter\tikz@child@edge@node@options\expandafter{\tikz@child@edge@node@options,name=#1}%
  \tikz@child@edge@node@notnamed%
}
\def\tikz@child@edge@node@notnamed#1{%
  \edef\tikz@childtime{node[style=tree edge node,pos=\tikz@time,}%
  \path [style=tree edge]\expandafter[\tikz@child@edge@options]
    \tikz@tree@edge@path 
    \expandafter\tikz@childtime\tikz@child@edge@node@options] {#1};
  \tikz@child@continueA%
}
\def\tikz@child@edge@nonode{%
