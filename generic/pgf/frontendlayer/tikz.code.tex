...
\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/Attic/tikz.code.tex,v 1.88 2007/07/12 21:54:08 tantau Exp $
...
\def\tikzoption#1{\pgfutil@ifnextchar[{\tikzoption@opt{#1}}{\tikzoption@noopt{#1}}}%} was : \tikz@orig@define@key{tikz}

\def\tikzoption@opt#1[#2]#3{\pgfkeysdef{/tikz/#1}{#3}\pgfkeyssetkey{/tikz/#1/.@def}{#2}}
\def\tikzoption@noopt#1#2{\pgfkeysdef{/tikz/#1}{#2}\pgfkeyssetkey{/tikz/#1/.@def}{\pgfkeysvaluerequired}}
...
\def\tikz@style@parseA#1#2=#3[#4]{% #2 and #3 are dummy
  \pgfutil@in@+{#2}%
  \ifpgfutil@in@%
    \pgfset{/tikz/#1/.append style={#4}}%
  \else%
    \pgfset{/tikz/#1/.style={#4}}%
  \fi}
...
\pgfset{/tikz/style/.style=#1}

\pgfset{/tikz/.unknown/.def=%
  \expandafter\pgfutil@in@\expandafter!\expandafter{\pgfkeyscurrentwithoutpath}%
  \ifpgfutil@in@%
    % this is a color!
    \expandafter\tikz@addoption\expandafter{\expandafter\pgfutil@color\expandafter{\pgfkeyscurrentwithoutpath}}%
    \edef\tikz@textcolor{\pgfkeyscurrentwithoutpath}%
  \else%
    \pgfutil@doifcolorelse{\pgfkeyscurrentwithoutpath}
    { %     
      \expandafter\tikz@addoption\expandafter{\expandafter\pgfutil@color\expandafter{\pgfkeyscurrentwithoutpath}}%
      \edef\tikz@textcolor{\pgfkeyscurrentwithoutpath}%
    }%
    {%
      ...
      \ifpgfutil@in@%
        % Ah, an arrow spec!
        \expandafter\tikz@processarrows\expandafter{\pgfkeyscurrentwithoutpath}%
      \else%
        % Ok, third chance: A shape!
        \expandafter\ifx\csname pgf@sh@s@\pgfkeyscurrentwithoutpath\endcsname\relax%
          \PackageError{tikz}{I do not know what to do with the option ``\pgfkeyscurrentwithoutpath''}{}
        \else%
          \edef\tikz@shape{\pgfkeyscurrentwithoutpath}%
        \fi%
      \fi%
    }%
  \fi%
...
\def\tikzpath@text{/tikz/}
\def\tikz@setkeys#1{\let\pgfkeysdefaultpath\tikzpath@text\pgfsetalso{#1}}% faster than \pgfset{tikz,#1}
\def\tikz@every@style#1{\ifpgfkeydefined{/tikz/#1/.@cmd}{\tikz@setkeys{#1}}{}}%
...
{%
  \pgfset{/tikz/cs/.use as path,name=,anchor=none,angle=none,#1}%
...
\pgfset{/tikz/cs/x/.store in=\tikz@cs@x}
\pgfset{/tikz/cs/y/.store in=\tikz@cs@y}
\pgfset{/tikz/cs/z/.store in=\tikz@cs@z}
\pgfset{/tikz/cs/angle/.store in=\tikz@cs@angle}
\pgfset{/tikz/cs/x radius/.store in=\tikz@cs@xradius}
\pgfset{/tikz/cs/y radius/.store in=\tikz@cs@yradius}
\pgfset{/tikz/cs/radius/.style={/tikz/cs/x radius=#1,/tikz/cs/y radius=#1}}
\pgfset{/tikz/cs/name/.store in=\tikz@cs@node}
\pgfset{/tikz/cs/anchor/.store in=\tikz@cs@anchor}

\pgfset{/tikz/cs/first line/.def args={(#1)--(#2)}{\def\tikz@cs@line@a@begin{(#1)}\def\tikz@cs@line@a@end{(#2)}}}
\pgfset{/tikz/cs/second line/.def args={(#1)--(#2)}{\def\tikz@cs@line@b@begin{(#1)}\def\tikz@cs@line@b@end{(#2)}}}

\pgfset{/tikz/cs/horizontal line through/.store in=\tikz@cs@hori@line}
\pgfset{/tikz/cs/vertical line through/.store in=\tikz@cs@vert@line}
