\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/Attic/tikz.code.tex,v 1.27 2005/10/03 20:00:24 tantau Exp $
...
\def\tikz@collect@child hild{%
  \futurelet\@let@token\tikz@collect@childA}
\def\tikz@collect@childA{%
  \ifx\@let@token r%
    \let\pgf@next=\tikz@collect@children@children%
  \else%
    \let\pgf@next=\tikz@collect@childB%
  \fi%
  \pgf@next%
}
\def\tikz@collect@childB{%
...
\def\tikz@collect@children@children ren{%
  \@ifnextchar[%
    \tikz@collect@children@children@opt%
    {\tikz@collect@children@children@opt[]}
}%}
\def\tikz@collect@children@children@opt[#1]{%
  \@ifnextchar\bgroup%
  {\tikz@ccc[#1]\tikz@internal@var in}
  {\tikz@ccc[#1]}}
\def\tikz@ccc[#1]#2in#3#4{%
