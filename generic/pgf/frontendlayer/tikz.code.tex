...
\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/Attic/tikz.code.tex,v 1.102 2007/12/13 18:40:19 tantau Exp $
...
% Shading options
\tikzoption{fading}{\def\tikz@fading{#1}\tikz@addmode{\tikz@mode@fadetrue}}
\tikzoption{fading angle}{\def\tikz@fade@angle{#1}\tikz@addmode{\tikz@mode@fadetrue}}
\def\tikz@fade@angle{0}
\colorlet{transparent}{pgftransparent}

\tikzoption{scope fading}{\tikz@addoption{\def\tikz@scope@fading{#1}}}
\tikzoption{scope fading bounding box}{\tikz@addoption{\def\tikz@scope@fading@bb{#1}}}
\let\tikz@scope@fading\pgfutil@empty
\let\tikz@scope@fading@path\pgfutil@empty
...
\tikzstyle{transparency group}=      [execute at begin scope={\pgftransparencygroup},
                                      execute at end scope={\endpgftransparencygroup}]
...
\def\tikz@scope@fade{%
  \ifx\tikz@scope@fading\pgfutil@empty%
  \else%
    \ifx\tikz@scope@fading@bb\pgfutil@empty%
      \pgfsetfading{\tikz@scope@fading}{\pgftransformrotate{\tikz@fade@angle}}%
    \else%
      \path%
        \tikz@scope@fading@bb%
        \pgfextra{\pgfsetfadingforcurrentpath{\tikz@scope@fading}{\pgftransformrotate{\tikz@fade@angle}}};%
    \fi%
  \fi%
}
...
\def\tikz@finish{%
  ...
  \iftikz@mode@fade%
    \tikz@addoption{%
      \pgfsetfadingforcurrentpath{\tikz@fading}{\pgftransformrotate{\tikz@fade@angle}}%
      \tikz@mode@fadefalse% no more fading...
    }%
  \fi%
  % Rendering pipeline 
  %
