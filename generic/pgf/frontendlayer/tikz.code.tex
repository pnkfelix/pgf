\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/Attic/tikz.code.tex,v 1.58 2006/09/14 16:56:15 tantau Exp $
...
\define@key{tikz}{step}{\tikz@handle@vec{\tikz@step@point}{\tikz@step@single}#1\relax}
...
% Pin options
\tikzoption{pin}{\tikz@parse@pin{}{#1}}

\tikzoption{pin above}{\tikz@parse@pin{pin angle=90}{#1}}
\tikzoption{pin below}{\tikz@parse@pin{pin angle=-90}{#1}}
\tikzoption{pin left}{\tikz@parse@pin{pin angle=180}{#1}}
\tikzoption{pin right}{\tikz@parse@pin{pin angle=0}{#1}}
\tikzoption{pin above left}{\tikz@parse@pin{pin angle=135}{#1}}
\tikzoption{pin below left}{\tikz@parse@pin{pin angle=-135}{#1}}
\tikzoption{pin above right}{\tikz@parse@pin{pin angle=45}{#1}}
\tikzoption{pin below right}{\tikz@parse@pin{pin angle=-45}{#1}}

\tikzoption{pin distance}{\def\tikz@pin@distance{#1}}
\tikzoption{pin angle}{\def\tikz@pin@angle{#1}}

\tikzoption{tikz@pin@post}[]{\tikz@compute@direction{\tikz@pin@angle}{\tikz@pin@distance}}

\def\tikz@pin@distance{4ex}
\def\tikz@pin@angle{0}

\def\tikz@parse@pin#1#2{\pgf@ifnextchar[{\tikz@@parse@pin#1}{\tikz@@parse@pin#1[]}#2\pgf@nil}%}
\def\tikz@@parse@pin#1[#2]#3\pgf@nil{%
  \tikz@add@after@node@path{\bgroup
    \pgfextra{\let\tikz@save@last@node=\tikzlastnode}%
    node
    [every pin,#1,#2,at=(\tikzlastnode.\tikz@pin@angle),%
    after node path={(\tikz@save@last@node) edge[every pin path] (\tikzlastnode)},
    tikz@pin@post]
    {#3} \egroup}
}
...
\tikzoption{label}{\tikz@parse@label{}{#1}}

\tikzoption{label above}{\tikz@parse@label{label angle=90}{#1}}
\tikzoption{label below}{\tikz@parse@label{label angle=-90}{#1}}
\tikzoption{label left}{\tikz@parse@label{label angle=180}{#1}}
\tikzoption{label right}{\tikz@parse@label{label angle=0}{#1}}
\tikzoption{label above left}{\tikz@parse@label{label angle=135}{#1}}
\tikzoption{label below left}{\tikz@parse@label{label angle=-135}{#1}}
\tikzoption{label above right}{\tikz@parse@label{label angle=45}{#1}}
\tikzoption{label below right}{\tikz@parse@label{label angle=-45}{#1}}

\tikzoption{label distance}{\def\tikz@label@distance{#1}}
\tikzoption{label angle}{\def\tikz@label@angle{#1}}

\tikzoption{tikz@label@post}[]{\tikz@compute@direction{\tikz@label@angle}{\tikz@label@distance}}

\def\tikz@label@distance{0pt}
\def\tikz@label@angle{0}

\def\tikz@parse@label#1#2{\pgf@ifnextchar[{\tikz@@parse@label#1}{\tikz@@parse@label#1[]}#2\pgf@nil}%}
\def\tikz@@parse@label#1[#2]#3\pgf@nil{%
  \tikz@add@after@node@path{\bgroup node [every label,#1,#2,at=(\tikzlastnode.\tikz@label@angle),tikz@label@post] {#3} \egroup}
}
...
\tikzstyle{every pin path}=          [help lines]
...
{%
  \setkeys{tikzcskeys}{name=,anchor=none,angle=none,#1}%
...
\define@key{tikzcskeys}{x}{\def\tikz@cs@x{#1}}
\define@key{tikzcskeys}{y}{\def\tikz@cs@y{#1}}
\define@key{tikzcskeys}{z}{\def\tikz@cs@z{#1}}
\define@key{tikzcskeys}{angle}{\def\tikz@cs@angle{#1}}
\define@key{tikzcskeys}{radius}{\def\tikz@cs@xradius{#1}\def\tikz@cs@yradius{#1}}
\define@key{tikzcskeys}{x radius}{\def\tikz@cs@xradius{#1}}
\define@key{tikzcskeys}{y radius}{\def\tikz@cs@yradius{#1}}
\define@key{tikzcskeys}{name}{\def\tikz@cs@node{#1}}
\define@key{tikzcskeys}{anchor}{\def\tikz@cs@anchor{#1}}

\define@key{tikzcskeys}{first line}{\tikz@parse@cs@line{tikz@cs@line@a}#1}
\define@key{tikzcskeys}{second line}{\tikz@parse@cs@line{tikz@cs@line@b}#1}
...
\define@key{tikzcskeys}{horizontal line through}{\def\tikz@cs@hori@line{#1}}
\define@key{tikzcskeys}{vertical line through}{\def\tikz@cs@vert@line{#1}}
