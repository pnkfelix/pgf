\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/Attic/tikz.code.tex,v 1.18 2005/08/29 22:11:20 tantau Exp $
...
% Tree options
\define@key{tikz}{edge to parent path}{\def\tikz@edge@to@parent@path{#1}}

\define@key{tikz}{level distance}{\tikz@parse@distances{level}#1 \pgf@stop}%
\define@key{tikz}{sibling distance}{\tikz@parse@distances{sibling}#1 \pgf@stop}%

\define@key{tikz}{grow function}{\let\tikz@grow=#1}
...
\def\tikz@parse@distances#1#2 {%
  \@ifnextchar o{\tikz@parse@distances@level{#1}#2}%
  {\expandafter\setlength\csname tikz#1distance\endcsname{#2}\@gobble}%
}
\def\tikz@parse@distances@level#1#2on level #3 \pgf@stop{%
  \expandafter\def\csname tikz@tree@#1@distance#3\endcsname{#2}%
}
...
\let\tikz@grow=\tikz@grow@below@lr
...
\tikzstyle{edge to parent}=         [draw]

\tikzstyle{grow downward}=          [grow function=\tikz@grow@below@lr]
...
% Syntax for edge to parent: 
% edge to parent [options]
\def\tikz@edgetoparent dge to parent{\@ifnextchar[\tikz@@edgetoparent{\tikz@@edgetoparent[]}}%}
\def\tikz@@edgetoparent[#1]{%
  \let\tikz@edge@to@parent@needed=\@empty%
  \tikz@node@is@a@labeltrue%
  \tikz@scan@next@command [style=edge to parent,#1] \tikz@edge@to@parent@path%
}
...
\long\def\tikz@children@collected{%
  \begingroup%
    ...
    \expandafter\ifx\csname tikz@tree@level@distance\the\tikztreelevel\endcsname\relax%
    \else%
      \tikzleveldistance=\csname tikz@tree@level@distance\the\tikztreelevel\endcsname%
    \fi%
    \expandafter\ifx\csname tikz@tree@sibling@distance\the\tikztreelevel\endcsname\relax%
    \else%
      \tikzsiblingdistance=\csname tikz@tree@sibling@distance\the\tikztreelevel\endcsname%
    \fi%
...
%   edge to parent[options] node {label text} node {label text}}
    ...
    \hbox\bgroup\bgroup%
        \pgfinterruptpath%
          \pgfscope%
            ...
            {%
              \def\tikz@edge@to@parent@needed{edge to parent}
...
\def\tikz@parse@child@node@coordinate ordinate#1(#2){%
  \@ifnextchar a{\tikz@p@c@n@c@at(#2)}{%
    \def\tikz@child@node@text{[shape=coordinate,name=#2]{}}%
    \tikz@parse@child@node@rest}}
