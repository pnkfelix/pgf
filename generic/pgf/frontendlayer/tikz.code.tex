\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/Attic/tikz.code.tex,v 1.13 2005/08/25 16:19:33 tantau Exp $
...
\newcount\tikz@num@children
\newcount\tikz@current@child
...
% Tree options
\define@key{tikz}{tree edge path}{\def\tikz@tree@edge@path{#1}}
...
\define@key{tikz}{sibling angle}{\edef\tikzsiblingangle{#1}}

\define@key{tikz}{child transformation function}{\let\tikz@perform@child@shifting=#1}


\def\tikz@tree@edge@path{(\tikzparentnode) -- (\tikzchildnode)}

\tikzleveldistance=15mm
\tikzsiblingdistance=15mm
\def\tikzsiblingangle{20}
...
\def\tikz@tree@line{%
  ...
  \advance\tikz@num@children by-1\relax%
  \pgf@xb=\tikz@num@children\pgf@xb\relax%
  \pgf@yb=\tikz@num@children\pgf@yb\relax%
  \advance\tikz@current@child by-1\relax%
  \pgf@xc=\tikz@current@child\pgf@xc\relax%
  \pgf@yc=\tikz@current@child\pgf@yc\relax%
...
\def\tikz@tree@one@child{\pgfpoint{0cm}{-\tikzleveldistance}}
\def\tikz@tree@left@child{\pgfpoint{-.5\tikzsiblingdistance}{-\tikzleveldistance}}
\def\tikz@tree@right@child{\pgfpoint{.5\tikzsiblingdistance}{-\tikzleveldistance}}


\def\tikz@tree@circle{%
  \advance\tikz@num@children by-1\relax%
  \advance\tikz@current@child by-1\relax%
  \c@pgf@counta=-\tikzsiblingangle\relax%
  \multiply\c@pgf@counta by\tikz@num@children\relax%
  \divide\c@pgf@counta by2\relax%
  \c@pgf@countb=\tikzsiblingangle\relax%
  \multiply\c@pgf@countb by\tikz@current@child\relax%
  \advance\c@pgf@counta by\c@pgf@countb\relax%
  \edef\pgf@temp{\noexpand\pgftransformrotate{\the\c@pgf@counta}
    \noexpand\pgftransformyshift{-\tikzleveldistance}}%
  \pgf@temp%
}

\let\tikz@perform@child@shifting=\tikz@tree@circle
...
\tikzstyle{every plot}=           []
\tikzstyle{every node}=           []
\tikzstyle{every children}=       []
\tikzstyle{every grandchildren}=  []
...
\tikzstyle{tree edge}=            [draw]
\tikzstyle{tree edge label}=      []
...
\def\tikz@invoke@collected@onpath{%
  ...
  \def\tikz@default@pos{\let\tikz@time=\@empty}%
}
...
\def\tikz@fig@boxdone{%
      ...
      \egroup%
    \@ifnextchar c{\tikz@fig@withchildren}{\tikz@fig@continue}}
...
% Syntax for trees:
% node {...} children [options] { \child ...; \child ...; }

\def\tikz@children hildren{\@ifnextchar[{\tikz@childrenA}{\tikz@childrenA[]}}%]
\def\tikz@childrenA[#1]#2{%
  \begingroup%
    \let\tikzparentnode=\tikz@last@fig@name%
    \let\tikz@options=\@empty%
    \let\tikz@transform=\@empty%
    \tikz@num@children=0\relax%
    \tikz@every@style{every children}%
    \tikz@setkeys{#1}%
    \tikz@transform%
    % Count the number of children
    \ifnum\tikz@num@children=0\relax%
      \let\pgf@next=\tikz@countkids%
      \pgf@next#2\child\pgf@stop%
    \fi%
    % Ok, got the number (more or less...)
    \tikz@current@child=0\relax%
    \let\child=\tikz@childnode%
    #2%
    \global\setbox\tikz@tempbox=\copy\tikz@figbox%
  \endgroup%
  \setbox\tikz@figbox=\box\tikz@tempbox%  
...
\def\tikz@countkids#1\child#2\pgf@stop{%
  \def\pgf@test{#2}%
  \ifx\pgf@test\@empty%
    \let\pgf@next=\@gobble%
  \else%
    \advance\tikz@num@children by1\relax%
  \fi%
  \pgf@next#2\pgf@stop%
}
...
% Syntax for child:
% \child[options] (name) {node}
%   edge[options] labelled[options] {text}
%   children[options] {...};

\def\tikz@childnode{%
  \advance\tikz@current@child by1\relax%
  \def\tikz@child@node@options{name=\tikzparentnode-\the\tikz@current@child}%
  \@ifnextchar[\tikz@childnode@opt{\tikz@childnode@opt[]}}%}
\def\tikz@childnode@opt[#1]{%
  \def\tikz@child@options{#1}%
  \@ifnextchar n{\tikz@child@a@node}{\tikz@child@a@node@opt [shape=coordinate]{}}}
\def\tikz@child@a@node node{%
  \@ifnextchar[\tikz@child@a@node@opt{\tikz@child@a@node@opt[]}}%}
\def\tikz@child@a@node@opt[#1]{%
  \expandafter\def\expandafter\tikz@child@node@options\expandafter{\tikz@child@node@options,#1}%
  \@ifnextchar({\tikz@childnode@name}{\tikz@childnode@continue}}%}
\def\tikz@childnode@name(#1){%
  \expandafter\def\expandafter\tikz@child@node@options\expandafter{\tikz@child@node@options,name=#1}%
  \tikz@childnode@continue%
}
\def\tikz@childnode@continue#1{%
  % Ok, options and name read
  \setbox\tikz@figbox=\hbox\bgroup%
    \unhbox\tikz@figbox%
    \hbox\bgroup\bgroup%
        \pgfinterruptpath%
          \pgfscope%
            \tikz@options% from children 
            \let\tikz@options=\@empty%
            \let\tikz@transform=\@empty%
            \expandafter\tikz@setkeys\expandafter{\tikz@child@options}%
            \tikz@options% from children 
            \tikz@transform%            
            \tikz@perform@child@shifting%
            % Typeset node:
            \expandafter\node\expandafter[\tikz@child@node@options]{#1}
              \pgfextra{\global\let\tikzchildnode=\tikz@last@fig@name};%
            % Transform to center of node
            \pgftransformshift{\pgfpointanchor{\tikzchildnode}{center}}%
  % Next, check for parent edge
  \let\tikz@child@edge@options=\@empty%
  \@ifnextchar t{\tikz@child@edge}{\tikz@child@edge@nolabel}%
}
\def\tikz@child@edge tree edge{\@ifnextchar[{\tikz@child@edge@opt}{\tikz@child@edge@opt[]}}%}
\def\tikz@child@edge@opt[#1]{%
  \def\tikz@child@edge@options{#1}%
  \@ifnextchar l{\tikz@child@edge@label}{\tikz@child@edge@nolabel}%
}
\def\tikz@child@edge@label label{\@ifnextchar[{\tikz@child@label@opt}{\tikz@child@label@opt[]}}%}%
\def\tikz@child@label@opt[#1]#2{%
  \edef\tikz@childtime{node[style=tree edge label,pos=\tikz@time,}%
  \path [style=tree edge]\expandafter[\tikz@child@edge@options] \tikz@tree@edge@path \tikz@childtime#1] {#2};
  \tikz@child@continueA%
}
\def\tikz@child@edge@nolabel{%
  \path [style=tree edge]\expandafter[\tikz@child@edge@options] \tikz@tree@edge@path;
  \tikz@child@continueA%
}
\def\tikz@child@continueA{\@ifnextchar c{\tikz@child@children}{\tikz@child@done}}
\def\tikz@child@children children{\@ifnextchar[{\tikz@child@children@opt}{\tikz@child@children@opt[]}}%}
\def\tikz@child@children@opt[#1]#2{%
  \let\tikz@last@fig@name=\tikzchildnode%
  \path children[#1,style=every grandchildren]{%
    #2};
  \tikz@child@done%
}
\def\tikz@child@done{%
    ...
    \egroup\egroup%
  \egroup%
  \@ifnextchar;{\@gobble}{\@ifnextchar\tikz@activesemicolon{\@gobble}{\PackageError{tikz}{Semicolon expected}}}%
}
