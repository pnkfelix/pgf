\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/Attic/tikz.code.tex,v 1.49 2006/05/26 21:34:19 tantau Exp $
...
% Baseline options
\define@key{tikz}{baseline}[0pt]{\pgf@ifnextchar({\tikz@baseline@coordinate}{\tikz@baseline@simple}#1\@nil}%)
...
\define@key{tikz}{path only}[]{\let\tikz@mode=\pgf@empty}
...
% Pattern options
\define@key{tikz}{pattern color}{\def\tikz@pattern@color{#1}}
\define@key{tikz}{pattern}[]{%
  \def\tikz@temp{#1}%
  \ifx\tikz@temp\tikz@nonetext%
    \tikz@addmode{\tikz@mode@fillfalse}%
  \else%
...
\define@key{tikz}{parent anchor}{\def\tikzparentanchor{.#1}\ifx\tikzparentanchor\tikz@border@text\let\tikzparentanchor\pgf@empty\fi}
\define@key{tikz}{child anchor}{\def\tikzchildanchor{.#1}\ifx\tikzchildanchor\tikz@border@text\let\tikzchildanchor\pgf@empty\fi}
...
\def\tikz@e@char{%
  \pgf@ifnextchar l{\tikz@ellipse}{\tikz@edgetoparent}}
...
\def\tikz@edgetoparent dge from parent{\pgf@ifnextchar[\tikz@@edgetoparent{\tikz@@edgetoparent[]}}%}
...
\def\tikz@fig@boxdone{%
      ...
      \egroup%
    \pgf@ifnextchar c{\tikz@fig@withchildren}{\pgf@ifnextchar[{\tikz@fig@withchildren}{\tikz@fig@continue}}}%}
...
\def\tikzdeclarecoordinatesystem#1#2{%
  \expandafter\def\csname tikz@parse@cs@#1\endcsname(##1){#2}%
}

\def\tikzreturncoordinate#1{\edef\tikz@return@coordinate{#1}}
...
{%
  ...
  \tikzreturncoordinate{\noexpand\pgfpoint{\tikz@cs@x}{\tikz@cs@y}}%
}

\tikzdeclarecoordinatesystem{linear}
{%
  \setkeys{tikzcskeys}{x=0,y=0,z=0,#1}%
  \tikzreturncoordinate{\noexpand\pgfpointxyz{\tikz@cs@x}{\tikz@cs@y}{\tikz@cs@z}}%
}

\tikzdeclarecoordinatesystem{polar}
{%
  \setkeys{tikzcskeys}{angle=0,radius=0,#1}%
  \tikzreturncoordinate{\noexpand\pgfpointpolar{\tikz@cs@angle}{\tikz@cs@radius}}%
}
...
\define@key{tikzcskeys}{radius}{\def\tikz@cs@radius{#1}}
