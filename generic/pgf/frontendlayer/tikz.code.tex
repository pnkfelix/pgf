...
\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/Attic/tikz.code.tex,v 1.89 2007/07/13 15:51:11 tantau Exp $
...
\def\tikzoption@opt#1[#2]#3{\pgfkeysdef{/tikz/#1}{#3}\pgfkeyssettokens{/tikz/#1/.@def}{#2}}
\def\tikzoption@noopt#1#2{\pgfkeysdef{/tikz/#1}{#2}\pgfkeyssettokens{/tikz/#1/.@def}{\pgfkeysvaluerequired}}
...
\pgfkeys{/tikz/.unknown/.def=%
  % Is it a pgf key?
  \let\tikz@key\pgfkeyscurrentwithoutpath% 
      ...
      {%
        ...
        \else%
          ...
          \expandafter\ifx\csname pgf@sh@s@\tikz@key\endcsname\relax%
            \PackageError{tikz}{I do not know what to do with the
            option ``\tikz@key''. I'm ignoring it}{}
          \else%
...
\long\def\tikz@children@collected{%
  \begingroup%
    ...
    \let\tikz@options=\pgfutil@empty%
    \let\tikz@transform=\pgfutil@empty%
    \tikzset{level \the\tikztreelevel/.try}%
    \tikz@transform%            
...
\tikzset{cs/first line/.def args={(#1)--(#2)}{\def\tikz@cs@line@a@begin{(#1)}\def\tikz@cs@line@a@end{(#2)}}}
\tikzset{cs/second line/.def args={(#1)--(#2)}{\def\tikz@cs@line@b@begin{(#1)}\def\tikz@cs@line@b@end{(#2)}}}
