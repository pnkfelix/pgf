...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/tikz/libraries/tikzlibrarychains.code.tex,v 1.2 2008/02/11 10:54:22 tantau Exp $
...
\def\tikz@lib@on@chain#1{%    
  ...
  \fi%
  \tikzset{after node path/.expanded={\noexpand\pgfextra{\noexpand\tikz@lib@after@last{\tikz@lib@chain@name}}},every on chain/.try}%
}
...
\def\tikz@lib@after@last#1{%
  \global\let\tikzchaincurrent=\tikz@last@fig@name%
  % Define pseudo nodes
  \ifnum\csname tikz@lib@chain@count@#1\endcsname=1\relax%
    % Define pseudostart
    \tikz@lib@alias@shape{#1-begin}{\tikz@last@fig@name}%
  \fi%
  % Define pseudoend
  \tikz@lib@alias@shape{#1-end}{\tikz@last@fig@name}%
}%

\def\tikz@lib@alias@shape#1#2{%
  \tikz@lib@namelet{pgf@sh@ns@#1}{pgf@sh@ns@#2}%
  \tikz@lib@namelet{pgf@sh@np@#1}{pgf@sh@np@#2}%
  \tikz@lib@namelet{pgf@sh@nt@#1}{pgf@sh@nt@#2}%
  \tikz@lib@namelet{pgf@sh@pi@#1}{pgf@sh@pi@#2}%
}
\def\tikz@lib@namelet#1#2{%
  \expandafter\let\expandafter\tikz@temp\csname #2\endcsname%
  \expandafter\global\expandafter\let\csname #1\endcsname=\tikz@temp%
}
