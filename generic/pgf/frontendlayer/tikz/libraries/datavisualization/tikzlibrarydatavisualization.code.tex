...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/tikz/libraries/datavisualization/tikzlibrarydatavisualization.code.tex,v 1.11 2008/12/13 23:23:17 tantau Exp $
...
\tikzdatavisualizationset{%
  ...
  common/.initial=,
  only ticks/.initial=,
  only minor ticks/.initial=,
  only subminor ticks/.initial=,
  ...
  every ticks/.style={styling/.style={line cap=round},min extend=-2pt,max extend=2pt},
  every minor ticks/.style={styling/.style={help lines,thin,line cap=round},min extend=-1.4pt,max extend=1.4pt},
  every subminor ticks/.style={styling/.style={help lines,line cap=round},min extend=-0.8pt,max extend=0.8pt},
  % Default grid styles
  only grid/.initial=,
  only minor grid/.initial=,
  only subminor grid/.initial=,
  every grid line/.style={max extend=max,min extend=min},
  every grid/.style={styling/.style={help lines,thin,black!25}},
  every minor grid/.style={styling/.style={help lines,black!25}},
  every subminor grid/.style={styling/.style={help lines,black!10}}
}
...
\tikzset{
  ...
  minor ticks/.default=auto,
  subminor ticks/.style={\tikz@dv@axis/subminor ticks={#1}},
  subminor ticks/.default=auto,
  visualize ticks/.code=\expandafter\tikz@lib@dv@tv\expandafter{\tikz@dv@axis}{#1},
  %
  % Grid settings
  %
  grid/.style={\tikz@dv@axis/grid={#1}},
  grid/.default=auto,
  minor grid/.style={\tikz@dv@axis/minor grid={#1}},
  minor grid/.default=auto,
  subminor grid/.style={\tikz@dv@axis/subminor grid={#1}},
  subminor grid/.default=auto,
  visualize
  grid/.code=\expandafter\tikz@lib@dv@gv\expandafter{\tikz@dv@axis}{#1},
  % 
...
\def\tikz@lib@dv@tv#1#2{
  \tikzdatavisualizationset{
    ticks actions/.append={\tikz@lib@dv@show@ticks{ticks}{#1}{#2}},
    minor ticks actions/.append={\tikz@lib@dv@show@ticks{minor ticks}{#1}{#2}},
    subminor ticks actions/.append={\tikz@lib@dv@show@ticks{subminor ticks}{#1}{#2}}
  }
}

\def\tikz@lib@dv@show@ticks#1#2#3{%
  % First, check whether there is anything to do at all:
  \pgfkeysgetvalue{#2/#1}\tikz@dv@ticks@settings%
  \ifx\tikz@dv@ticks@settings\pgfutil@empty%
    % Great, nothing to do
  \else
    {%
      \scope[/tikz/data visualization/.cd,every tick/.try,every #1/.try,#3]
        % Setup options
        \def\tikz@dv@tick@tempa{\pgfqkeys{/tikz/data visualization}}
        \pgfkeysgetvalue{/tikz/data visualization/common}\tikz@dv@tick@tempb
        \expandafter\tikz@dv@tick@tempa\expandafter{\tikz@dv@tick@tempb}
        \pgfkeysgetvalue{/tikz/data visualization/only #1}\tikz@dv@tick@tempb
        \expandafter\tikz@dv@tick@tempa\expandafter{\tikz@dv@tick@tempb}
        \expandafter\tikz@dv@tick@tempa\expandafter{\tikz@dv@ticks@settings}
        ...
        \foreach \tikz@dv@tick@pos/\tikz@dv@tick@text in \tikz@dv@tick@list 
        {
...
\def\tikz@dv@handle@tick#1{%
  \pgfextra{
    \expandafter\tikz@dv@parse@tick\tikz@dv@tick@text\pgf@stop%
    \ifx\tikz@dv@tick@text\pgfutil@empty%
      \global\tikz@dv@tick@marshal=\pgfutil@empty% Skip!
    \else
      \def\tikz@dv@tick@marshal{node[anchor=#1,every tick node/.try,}%
      \expandafter\pgfutil@g@addto@macro\expandafter\tikz@dv@tick@marshal\expandafter{\tikz@dv@tick@options]}
      \expandafter\pgfutil@g@addto@macro\expandafter\tikz@dv@tick@marshal\expandafter{\expandafter{\tikz@dv@tick@text}}
    \fi
  }%
  \tikz@dv@tick@marshal
}       

\def\tikz@dv@parse@tick{%
  \pgfutil@ifnextchar[{\tikz@dv@parse@@tick}{\tikz@dv@parse@@tick[]}%]
}
\def\tikz@dv@parse@@tick[#1]#2\pgf@stop{%
  \def\tikz@dv@tick@options{#1}%
  \def\tikz@dv@tick@text{#2}%
}
...
\def\tikz@lib@dv@gv#1#2{
  \tikzdatavisualizationset{
    grid actions/.append={\tikz@lib@dv@show@grid{grid}{#1}{#2}},
    minor grid actions/.append={\tikz@lib@dv@show@grid{minor grid}{#1}{#2}},
    subminor grid actions/.append={\tikz@lib@dv@show@grid{subminor grid}{#1}{#2}}
  }
}

\def\tikz@lib@dv@show@grid#1#2#3{%
  % First, check whether there is anything to do at all:
  \pgfkeysgetvalue{#2/#1}\tikz@dv@grid@settings%
  \ifx\tikz@dv@grid@settings\pgfutil@empty%
    % Great, nothing to do
  \else
    {%
      \scope[/tikz/data visualization/.cd,every grid line,every #1/.try,#3]
        % Setup options
        \def\tikz@dv@grid@tempa{\pgfqkeys{/tikz/data visualization}}
        \pgfkeysgetvalue{/tikz/data visualization/common}\tikz@dv@grid@tempb
        \expandafter\tikz@dv@grid@tempa\expandafter{\tikz@dv@grid@tempb}
        \pgfkeysgetvalue{/tikz/data visualization/only #1}\tikz@dv@grid@tempb
        \expandafter\tikz@dv@grid@tempa\expandafter{\tikz@dv@grid@tempb}
        \expandafter\tikz@dv@grid@tempa\expandafter{\tikz@dv@grid@settings}
        \pgfkeysgetvalue{/tikz/data visualization/at}\tikz@dv@grid@list
        \pgfkeysgetvalue{#2/scaling mapper}\pgf@dv@grid@mapper
        ...
        \foreach \tikz@dv@grid@pos in \tikz@dv@grid@list 
        {
          \pgf@dv@grid@mapper.set in to(\tikz@dv@grid@pos)%
          \draw[/tikz/data visualization/styling]
            \pgfextra{
...
\def\tikz@lib@dv@lv#1#2{
  \tikzdatavisualizationset{
    axes actions/.append={\tikz@lib@dv@show@label{#1}{#2}}
  }
}
...
\def\tikz@lib@dv@show@label#1#2{%
  ...
  \else
    \scope[/tikz/data visualization/.cd,every label/.try,#2]
      \pgfkeysgetvalue{/tikz/data visualization/at}\tikz@lib@dv@at%
...
\def\tikz@lib@dv@parse@node@opt[#1]#2\pgf@stop{
  \node [every axis node/.try,/tikz/data visualization/styling,#1] {#2};
}
...
\tikzdatavisualizationset{
  school book plot/.style={
    xy Cartesian,
    x axis={
      visualize axis={y axis={goto=0},styling/.style=->},
      visualize ticks={common={y axis={goto=0},direction axis=y axis},only ticks={tick text at min extend}},
      visualize grid={common={direction axis=y axis}},
      ticks,
      min extend=-.666em,
      max extend=.666em,
      visualize label={at=max,y axis={goto=0}},
    },
    y axis={
      visualize axis={x axis={goto=0},styling/.style=->},
      visualize ticks={common={x axis={goto=0},direction axis=x axis},only ticks={tick text at min extend}},
      visualize grid={common={direction axis=x axis}},
      ticks,
      min extend=-.666em,
      max extend=.666em,
      visualize label={at=max,x axis={goto=0}},
    },
    new line plot,
...
% The 3d version of the school book plot
...
% The scientific plot
%
% The scientific plot is a 2d plot that has a predetermined width and
% height. The data is then scaled in such a way that it fits inside
% this given rectangle. 

\tikzdatavisualizationset{
  scientific plot/width/.initial=125pt,
  scientific plot/height/.initial=0.618*125pt,% golden ratio...
  scientific plot/.style={
    xy Cartesian,
    every axis/.style={styling/.style={thin,black!50}},
    every ticks/.append style={styling/.style={thin,black!50}},
    x axis={
      length=\pgfkeysvalueof{/tikz/data visualization/scientific    plot/width},
      ticks,
      visualize ticks={common={y axis={goto=min},direction axis=y axis,min extend=0pt},only ticks={tick text at min extend}},
      ...
      visualize grid={common={direction axis=y axis}},
      visualize label={pos=.5,styling/.style={below=1em},y axis={goto=min}},
    },
    y axis={
      length=\pgfkeysvalueof{/tikz/data visualization/scientific plot/height},
      ticks,
      visualize ticks={common={x axis={goto=min},direction axis=x axis,min extend=0pt},only ticks={tick text at min extend}},
      ...
      visualize grid={common={direction axis=x axis}},
      visualize label={pos=.5,styling/.style={xshift=-1em,rotate=90,above},x axis={goto=min}},
    },
    new line plot,
  }
}

% The 3d version. Here, a "length" is used for the length of the three
