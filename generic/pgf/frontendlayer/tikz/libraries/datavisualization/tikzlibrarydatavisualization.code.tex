...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/tikz/libraries/datavisualization/tikzlibrarydatavisualization.code.tex,v 1.26 2010/06/18 11:59:28 tantau Exp $
  ...
  new visualizer/.style 2 args={
    ...
    /data point/#1/execute at begin/.initial={
      \scope[
        ...
        /tikz/data visualization/visualizers/#1/style/.try]
    },
    /data point/#1/execute at end/.initial={
        \path; % invokes every path implicitly
      \endscope
    },
    /tikz/data visualization/visualizers/#1/style/.style=,
  },
...
\pgfkeys{
  ...
  label/.code=\tikzdatavisualizationset{new label visualizer={#1}},
  style/.style={/tikz/data visualization/visualizers/\tikz@visualizer/style/.append style={#1}},
}
...
\tikzset{
  ...
  no lines/.style={@set plot handler=\pgfplothandlerdiscard},
  smooth line/.style={@set plot handler=\pgfplothandlercurveto},
  smooth cycle/.style={@set plot handler=\pgfplothandlerclosedcurve},
  straight line/.style={@set plot handler=\pgfplothandlerlineto},
  straight cycle/.style={@set plot handler=\pgfplothandlerpolygon},
  polygon/.style={straight cycle=cycle},% alias
  gapped line/.style={@set plot handler=\pgfplothandlergaplineto},
  gapped cycle/.style={@set plot handler=\pgfplothandlergapcycle}
}
...
\tikzdatavisualizationset{
  new label visualizer/.style={
    new object={
      when=after survey,
      class=label visualizer,
      before creation={
        \begingroup
        \pgfkeys{/tikz/data visualization/visualizer label options/.cd,#1}
        \pgfkeysgetvalue{/tikz/data visualization/visualizer label options/text}\tikz@dv@temp@lab
        \pgfkeysgetvalue{/tikz/data visualization/visualizer label options/styling}\tikz@dv@temp@style
        \def\tikz@tempa{
          \path [/tikz/data visualization/.cd,every visualizer label]
        ...
        \expandafter\expandafter\expandafter\def%
        \expandafter\expandafter\expandafter\tikz@tempb%
        \expandafter\expandafter\expandafter{%
          \expandafter\tikz@dv@temp@style\expandafter]\expandafter{\tikz@dv@temp@lab};}%
        ...
        \expandafter\expandafter\expandafter{\expandafter\expandafter\expandafter\relax%
          \expandafter\expandafter\expandafter{\expandafter\tikz@tempa\tikz@tempb}}
      ...
      arg4/.expanded=\noexpand\pgfdvvisualizerfilter{\tikz@visualizer}
    },
    /utils/exec=\expandafter\tikz@create@label@count\expandafter{\tikz@visualizer}
  }
}
...
\def\tikz@create@label@count#1{
  \tikzdatavisualizationset{
    new object={
      ...
      class=count,
      arg1=/tikz/data visualization/visualizers/#1/count,
      after creation={
        \tikzdvobj.set filter({%
...
\pgfkeys{
  ...
  index/.code={%
    \edef\tikz@dv@label@attribute{/tikz/data visualization/visualizers/\tikz@visualizer/count}%
    \def\tikz@dv@label@threshold{#1}%
  },
  text/.initial=,
  text'/.style={text={#1},style=swap},
  styling/.initial=,
  style/.style={styling/.append={,#1}}
}
...
\tikzdatavisualizationset{
  line plot/.style={visualize as line=line,/data point/visualizer=line},
  smooth line plot/.style={visualize as smooth line=line,/data point/visualizer=line},
  scatter plot/.style={visualize as scatter=scatter,/data point/visualizer=scatter},
}
...
\tikzdatavisualizationset{
  ...
  style sheet/colors/.code={
    \pgfkeysgetvalue{/tikz/data visualization/style sheet/colors/#1}\pgf@temp
    \tikz@dv@color@seriesfalse
    \ifx\pgf@temp\relax%
      \tikz@dv@color@seriestrue
    \else
      \ifx\pgf@temp\pgfutil@empty
        \tikz@dv@color@seriestrue
      \fi
    \fi
    \iftikz@dv@color@series
    ...
    \else
      \pgfkeysalso{%
        style sheet/visualizer styles/.expand once=\pgf@temp
      }%
    \fi
  },
  style sheet/colors/.default=hue,
  style sheet/colors/strong/.initial={
    color=black,
    color=red!80!black,
    color=blue!80!black,
  ...
  style sheet/marks/.is choice,
  style sheet/marks/crosses/.style={
    style sheet/visualizer styles={
      {mark=x,mark options=thin,mark size=1.8pt},
      {mark=+,mark options=thin,mark size=1.8pt},
      {mark=Mercedes star,mark options=thin,mark size=1.8pt},
      {mark=Mercedes star flipped,mark options=thin,mark size=1.8pt},
      {mark=star,mark options=thin,mark size=1.8pt},
      {mark=10-pointed star,mark options={line width=.3pt,mark size=1.6pt}}
    }
  },
  style sheet/marks/.default=crosses
}
