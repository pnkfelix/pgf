...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/tikz/libraries/tikzlibrarymindmap.code.tex,v 1.6 2008/05/15 09:08:40 tantau Exp $
...
\def\tikz@parse@switch from (#1) to (#2)\pgf@unique{%
  \def\tikz@to@path{%
    ...
    [every circle connection bar]
    decorate [decoration=circle connection bar]
    {\pgfextra{\pgf@resetpathsizes} -- (\tikztotarget) \tikztonodes} 
...
\pgfdeclarehorizontalshading[tikz@switch@from,tikz@switch@to]{tikz@shade@bar}{100bp}{%
  ...
  color(33bp)=(tikz@switch@from);
  color(67bp)=(tikz@switch@to);
