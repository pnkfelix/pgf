...
\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/tikz/tikz.code.tex,v 1.14 2008/01/27 19:31:25 tantau Exp $
...
{%
  \tikzset{cs/.cd,#1}%
  \expandafter\tikz@scan@one@point\expandafter\tikz@parse@linea\tikz@cs@line@a%
  \expandafter\tikz@scan@one@point\expandafter\tikz@parse@lineb\tikz@cs@line@b%
  \edef\pgf@marshal{%
...
\def\tikz@parse@linea#1--{%
  \pgf@process{#1}%
  \pgf@xa=\pgf@x%
  \pgf@ya=\pgf@y%
  \tikz@scan@one@point\tikz@parse@linea@second%
}
\def\tikz@parse@linea@second#1{%
  \pgf@process{#1}%
  \pgf@xb=\pgf@x%
  \pgf@yb=\pgf@y%
}

\def\tikz@parse@lineb#1--{%
  \pgf@process{#1}%
  \pgf@xc=\pgf@x%
  \pgf@yc=\pgf@y%
  \tikz@scan@one@point\tikz@parse@lineb@second%
}
\def\tikz@parse@lineb@second#1{%
  \pgf@process{#1}%
}
...
\tikzset{cs/first line/.store in=\tikz@cs@line@a}
\tikzset{cs/second line/.store in=\tikz@cs@line@b}
