% $Header: /cvsroot/pgf/pgf/doc/pgf/en/Attic/pgfmanual-guidelines.tex,v 1.1 2005/06/29 12:14:02 tantau Exp $
...
The first thing you ask yourself when someone presents a bunch of
...
Quite differently, a standalong figure will often be viewed at a time
...
(about the same size and the same numbers
are shown). In addition, one can directly ``see'' that there are more
good or very good ratings than bad ones. One can also ``see'' that the
number of people who gave no rating at all is not negligible (though
not uncommon for feedback forms).

However, charts are not always a good idea. Let us look at an example
