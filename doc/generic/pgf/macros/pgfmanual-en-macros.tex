...
% $Header: /cvsroot/pgf/pgf/doc/generic/pgf/macros/pgfmanual-en-macros.tex,v 1.19 2007/09/18 09:33:09 vibrovski Exp $


\providecommand\href[2]{\texttt{#1}}
\providecommand\hypertarget[2]{\texttt{#1}}% MW
\providecommand\hyperlink[2]{\texttt{#1}}% MW
...
% MW: This is used to split a shape name (if it contatins a space). 
%     It also creates a hyperlink (see shape envronment below). It
%     is only needed for the shape library documentation.
% 
\makeatletter

\def\splitshapename#1{\expandafter\splitshapename@#1 . \@@}
\def\splitshapename@#1 #2 #3\@@{%
	\ifx#2.\relax%
		\hyperlink{shape library:#1}{#1}%
	\else%
  	\hyperlink{shape library:#1 #2}{%
  		\hbox{%
  			\vbox{%
  				\halign{\hfil##\hfil\cr#1\cr#2\cr}%
  			}%
  		}%
  	}%
  \fi%
...
\newenvironment{shape}[1]{
  \begin{pgfmanualentry}
  	\hypertarget{shape library:#1}{}% MW
...
\def\extractkey#1\@nil{%
  ...
  \else%
    ...
    \else
      \pgfmanualentryheadline{{\ttfamily\declare{#1}}\hfill(no value)}%
      \def\mykey{#1}%
      \def\mypath{}%
      \def\myname{}%
      \firsttimetrue%
...
\def\extractkeyequal#1=#2\@nil{%
  ...
  \else%
    ...
    \else
      \pgfmanualentryheadline{{\ttfamily\declare{#1}=}#2\hfill(no default)}%
    \fi%
  \fi%
  \def\mykey{#1}%
  \def\mypath{}%
...
\def\extractdefault#1#2(default #3)\@nil{%
  \pgfmanualentryheadline{{\ttfamily\declare{#1}\opt{=}}\opt{#2}\hfill (default {\ttfamily#3})}%
}

\def\extractinitial#1#2(initially #3)\@nil{%
  \pgfmanualentryheadline{{\ttfamily\declare{#1}=}#2\hfill (initially {\ttfamily#3})}%
}

\def\extractequalinitial#1 (initially #2)\@nil{%
  \pgfmanualentryheadline{{\ttfamily\declare{#1}}\hfill (initially {\ttfamily#2})}%
  \def\mykey{#1}%
  \def\mypath{}%
  \def\myname{}%
  \firsttimetrue%
...
\def\itemstyle#1{\item \declare{\texttt{#1}}%
